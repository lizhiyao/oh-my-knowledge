...
- 首次自主 CR 前完整阅读 CODE_REVIEW.md；同一任务规则未变化时复用。以审查者视角只读检查最终变更、直接调用边界、契约和验证证据；发现跨模块影响时沿证据扩大范围。可使用独立 reviewer，否则做 fresh-pass 自审。
...
- artifacts/cr/ 和 knowledge/cr/ 不是源码资产，不得在仓库内创建或保留，也不用 .gitignore 隐藏。纯只读审查不创建过程文件；确需运行写盘工具时，用唯一仓库外临时目录保存全部运行产物，并显式配置输出路径，不执行回写仓库的结果同步。
