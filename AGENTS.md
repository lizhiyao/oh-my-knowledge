...
- 同一台机器上有需要互不干扰的并发工作时（例如另一份工作正持有未提交改动），为一份任务使用一份仓库外隔离检出：不在共享检出里跑重验证，不为不同阶段开出同属一份任务的多份检出，交付后销毁该检出并同步清理其登记。
- 每份检出使用独立的依赖与增量构建缓存。共用缓存会让缺产物的检出以退出码 0 假通过，这类构建结果不能当门禁证据；按条目软链依赖省安装时间可以，缓存目录必须留在该检出本地，机制与实测数字见 `CONTRIBUTING.md`。
