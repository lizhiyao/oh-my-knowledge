...
- 本地验证按风险分层：普通局部修改用 `ci:quick` 精准验证；跨模块、依赖、构建／打包、CI 门禁变化必须本地完整 `yarn ci`；测试依赖编译或生产资源时先更新对应构建产物再测试。首次 push 前在最终改动集上默认跑一次 `yarn ci`；例外仅为用户明确要求更窄验证，或不影响生成产物、链接、打包与行为的手写文档／规则。`ci:quick` 不替代完整门禁。
