...
user-invocable: true
argument-hint: "<clean|doctor|eval|evolve|init|install|list|migrate|observe|promote|rollback|sample|studio> [options]"
---
...
omk CLI 顶层命令包括：`init` / `migrate` / `clean` / `install` / `list` / `promote` / `rollback` / `doctor` / `eval` / `observe` / `evolve` / `sample` / `studio`。没有 `bench` / `improve` / `gen-samples` 这些旧子命令名 —— 如果你在历史 SKILL / 文档里看到了，那是 v0.30 命令树重构之前的写法。
...
| 升级旧 `.omk` 存储布局 | → `omk migrate`（先 `--dry-run`） |
