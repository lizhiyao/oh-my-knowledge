...
- `--from-traces` `boolean`:from-traces 模式：从 observe inbox 的失败信号或人工确认的 Knowledge Gap 回流生成评测用例草稿，落草稿待人工 review。
- `--gap` `option`:仅从指定 Knowledge Gap 生成关联草稿（仅 from-traces 模式用）。
- `--lang` `option` (默认 `zh`):输出语言 zh|en，优先级 CLI > OMK_LANG env > zh。
