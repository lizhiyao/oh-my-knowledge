...
- **Evaluation Core decision gate**: every candidate is measured as a fresh control／treatment A/B run and accepted only when Core returns `PROGRESS` with `release-gates-passed`. Core Decision is the sole admission authority; the same-run score delta is presentation data, and the authoring loop adds neither a private score gate nor a cross-run score comparison. Runtime, evidence, comparability, uncertainty, and release-policy failures therefore fail closed.
