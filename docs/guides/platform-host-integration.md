...
Read fine-grained scoring progress from the `evaluation.run.progress` event, whose payload is `{ completed, total, failed, maxConcurrency, retry, timeoutMs? }`: `total` is every coordinate planned for the run (including not-evaluated ones), `completed` and `failed` increase monotonically as coordinates settle, and `maxConcurrency`／`retry`／`timeoutMs` carry the scoring stage's execution controls, with `timeoutMs` absent when unconfigured. `EvaluateInput.policy.eventDelivery.progressGranularity` sets the cadence: `per-batch` (default, one event per concurrency-wide batch), `per-coordinate` (one event per settled coordinate), or `start-end-only` (no intermediate progress, only `evaluation.run.started` and the terminal event). Omitting the field keeps `per-batch` and writes nothing into the sealed plan, so existing plan digests are unchanged; progress events are process records and never change a measurement end state. A progress write that fails under `fail-run` stops the run from scheduling further batches, yet records the Evaluator already produced stay in the Bundle — a measured coordinate is never rewritten as not-evaluated evidence.
