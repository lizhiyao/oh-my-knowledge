...
- **`required` without an injected writer fails closed before any Target call.** Every Core stage checks the pairing: `evaluate()` and `prepared.run()` resolve with `status: 'failed'` and `error.code` `EXECUTION_RUNTIME_EVENT_WRITER_REQUIRED` at the `configuration` stage, while `rescore`/`reanalyze`/`redecide` reject with `EVAL_RUNTIME_REUSE_INVALID`, because the reuse boundary reports the same Core configuration failure under its own code. The mirror case also fails closed: supplying a writer while delivery is `disabled` rejects with `EVAL_RUNTIME_INPUT_INVALID` during preparation.
