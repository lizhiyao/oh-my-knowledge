...
The recommended ordinary-user entry. It exposes exactly the same canonical Runtime façade as `oh-my-knowledge/eval-runtime`: `evaluate`, `prepareEvaluation`, `evaluateSeries`, `prepareEvaluationSeries`, `rescore`, `reanalyze`, `redecide`, `assessComparability`, `checkExecutor`, `checkContentStore`, their stable errors, and their public model types. Core engines, builders, registrations, and adapters are intentionally absent.
...
Use `oh-my-knowledge/eval-core` for custom analysis graphs, persisted artifact admission, staged replay, transported cross-process comparability, or custom comparability policies. Deep implementation imports are unsupported.
