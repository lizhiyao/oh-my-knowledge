...
`EvaluationResult` preserves every field of the Core `EvaluationRunResult` and adds `definition`, `policy`, and `analysisResults`. The last field is a read-only `analysisId` index over the exact Core Analysis records, not a second analysis implementation. Execution and evaluation evidence remain under `artifacts`, the decision remains under `artifacts.decision`, and the public report remains under `report`.

`SamplingDesign` supports a one-Variant `solo` quality profile, complete-block `paired` comparisons, and fixed-quota `independent` comparisons. A solo design may declare `clusterKey`; Core then treats whole clusters as the experimental and resampling unit. A `Comparison` declares one control, one or more treatments, and the Metrics to analyze. `evaluators` may contain multiple exact-match, Rubric Judge, or custom evaluators, provided evaluator and metric IDs are unique.
...
Analysis is always explicit and preregistered. `analysis.analyses[]` accepts named `summary`, `quality-interval`, and `comparison-interval` requests. Summaries expose numeric `mean`, boolean `rate`, and numeric `quantile`; intervals use an explicit confidence level and resample count. Each request selects one Variant or one declared comparison contrast and may apply a sealed Dataset cohort filter. Rubric panels aggregate replicates within members, members by the declared rule, and repeated Target trials within a sample before summary or bootstrap. Cluster bootstrap resamples complete clusters. Metric direction is never used to silently flip a signed result. `decision` selects one interval by `analysisId`; summaries and lower-is-better Metrics cannot silently drive `progress/v2`. An empty analysis list intentionally retains typed evaluation evidence without fabricating statistics or a composite verdict.
