...
`SamplingDesign` supports a one-Variant `solo` quality profile, complete-block `paired` comparisons, and fixed-quota `independent` comparisons. A `Comparison` declares one control, one or more treatments, and the Metrics to analyze. `evaluators` may contain multiple exact-match, Rubric Judge, or custom evaluators, provided evaluator and metric IDs are unique.
...
Analysis is always preregistered. For numeric and boolean Metrics, the façade creates one mean-bootstrap result per Metric for `solo`, or one raw treatment-minus-control difference-bootstrap result per comparison × treatment × Metric: paired for `paired`, unpaired for `independent`. Rubric panels first average replicates within each member, apply the declared member aggregation, and then average complete Target trials within a sample. Bootstrap `unitCount` still counts samples or paired blocks; judge members and replicates are measurements, not independent statistical units. The Metric direction remains attached for interpretation; lower-is-better results are not silently sign-flipped. Categorical, text, and ranking Metrics remain typed evaluation evidence until a compatible estimator is explicitly designed; the façade does not pretend they are numeric. It does not fabricate a composite verdict. Omitting `decision` returns all analysis results without a Decision; supplying `decision` explicitly selects exactly one analyzable, higher-is-better result for the interval-aware Core `progress/v2` policy.
