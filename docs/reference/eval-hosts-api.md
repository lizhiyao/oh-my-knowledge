...
1. **Configuration capture.** `executablePath` must be absolute and free of NUL; `model` non-empty; `sandbox` is one of `read-only`／`workspace-write` and `effort` one of `low`／`medium`／`high`／`xhigh`／`max`, each rejected here rather than deferred to the vendor; limits positive integers; every `CodexCliEnvironmentEntry` validated and classified. A `credential` entry raises output and trace handling to `secret`, and the floor for this seam is `sensitive` — `outputTaint` only ever raises that pairing, so a declared taint cannot pull a credential back down; the value never enters an identity facet, only a digest of the classified entry does.
