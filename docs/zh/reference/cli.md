...
```text
  ...
  --executor <value>              执行器：claude / claude-sdk / codex / codex-sdk / dsh / openai-api / gemini / 自定义命令。Codex 任务内自动用 codex；也可用 OMK_EXECUTOR 设置环境偏好。
  ...
  --judge-models <value>          评委配置，格式 executor:model[,...]，例 claude:haiku、codex:<model> 或 dsh:<model>（≥ 2 个 = ensemble）。默认跟随所选执行器；Codex／DSH 沿用被测模型。
