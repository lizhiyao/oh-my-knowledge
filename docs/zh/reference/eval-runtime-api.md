...
| `createFormulaEvaluator` | 从 calculator ID 和字段映射创建声明式固定公式评分器；binding 名称必须与 calculator 声明的输入集合精确一致，算不出来的输入产生带 `reasonCode` 的 missing／invalid 观测，不伪造分值。 |
| `FORMULA_CALCULATOR_IDS` | 内置 formula calculator 标识符，各自的必填 binding 与口径为：`ratio`（`numerator` ÷ `denominator`）、`ranking`（`target` 在 `candidates` 中的 1 基名次）、`duration`（`endMs` − `startMs`）、`mean`（`values` 的算术均值）、`percentage`（`value` × 100）。 |
...
Builder type 包括 `ExactMatchDefinitionBuilderInput`、`ExactMatchTarget`、`PairedComparisonDefinitionBuilderInput`、`EvaluationRuntimeTarget`、`MeasurementPolicyBuilderInput`、`MeasurementStagePolicyInput`、`MeasurementRetryPolicyInput`、`MeasurementRetryBackoffInput`、`MeasurementFailurePolicyInput`、`MeasurementEvidencePolicyInput`、`MeasurementEventDeliveryInput`、`MeasurementBudgetPolicyInput`、`MeasurementBudgetScopeInput`、`MeasurementAttemptBudgetScopeInput`、`MeasurementRunBudgetScopeInput`、`MeasurementCachePolicyInput`、`MeasurementProviderCostLimitInput`、`CreateExactMatchEvaluatorInput`、`CreateRubricJudgeKitInput`、`RubricJudgeKit`、`CreateRubricJudgeEvaluatorInput`、`RubricJudgeEvaluatorBinding` 与 `RubricJudgeEvaluatorDefinitionBuilderInput`。Formula evaluator type 包括 `FormulaCalculatorId`、`FormulaEvaluator`、`FormulaEvaluatorBinding` 与 `FormulaEvaluatorConfig`。
...
Executor conformance probe 自行密封它的测量设计，因此被测声明不必去迎合 probe 假设的前提：deterministic 的 Executor，或能控制 seed 的 Executor，仍在 paired 的 `shared-within-block` 采样设计下认证；而声明 `seedControl: 'unsupported'` 的随机 Executor，则改用 `seedCoupling: 'uncontrolled'`——与产品宿主为这类 Target 编译的耦合方式一致。两种情况下 11 项 `RuntimeConformanceCheck` 的 id 与证据完全一致——只有 `experiment.sampling.seedCoupling` 不同——直接调用 `runExecutorConformance` 的宿主仍然可以用 `ExecutorConformanceProbeInput.seedCoupling` 显式钉住该设计。
...
`RubricJudgeEvaluator` 是一份显式评委 panel。`judges` 包含一个或多个 `RubricJudgeMember`；`replicateCount` 只重复该成员的测量，不会重新执行 Target。单个 panel 最多可展开为 1000 个 member × replicate 坐标。`RubricJudgeAggregation` 必须选择成员等权的 `mean`，或权重完整覆盖所有成员、均为正数且总和为 1 的 `weighted-mean`。目前唯一支持的缺失规则是 `require-complete`：某指标的任一计划成员或 replicate 不可用时，该指标对应 Target × Sample × Trial 的 panel 读数不会进入分析，更不会用剩余评委补平均；原始成员与 replicate record 仍保留在 Evaluation Bundle 中。

`CustomEvaluator` 声明非空 `metrics: Metric[]`、显式输入 `bindings`、可序列化 `parameters` 与测量相关 identity facets。`implementation.schemas.values` 按每个声明的指标 ID 提供 parser。Callback 只接收选中的 bindings，在每个 Sample × Variant × Trial 的一次评分尝试中调用一次，返回 `{ resultKind: 'completed', results: CustomMetricResult[], usage? }`。每个指标必须恰好出现一次，带 `metricId` 和 `score`、`missing` 或 `invalid` 结果，顺序不限。分数校验失败只影响对应指标；缺失／无效原因、带 classification 的证据、覆盖率与汇总独立保留。空结果、遗漏、重复或未知指标 ID 会以 `custom-evaluator-result-invalid` 使整次调用失败。稳定的 `{ resultKind: 'failed', errorCode, usage? }` 同样作用于整次调用。Usage 属于调用，只计一次。并发、超时、重试、预算、取消、证据捕获和错误脱敏由 Core 统一管理，调用级控制作用于整个指标组。Callback 必须无状态、可安全并行且协作响应取消；有状态集成使用 `/advanced`，custom evaluator 不能自行声明 ensemble coordinate。

**Rubric Judge v2 迁移 — BREAKING-COMPARABILITY。** <a id="rubric-judge-v2-migration"></a>

将 façade 的 `metricId` 与 `rubric` 改为 `rubrics: [{ metricId, ...rubric }]`；单维调用使用单元素数组。`RubricJudgeDimension` 包含 `metricId`、`criterionId`、`prompt`、`rubric`，两组 ID 都必须唯一。`createCodexCliReferenceEvaluator` 同步迁移，不接受旧字段形态。

每个 member × replicate 对全部维度只调用一次 provider。提示词要求返回 `{"scores":[{"metricId":"quality","score":4,"reason":"证据","reasoning":"可选解释"}]}`。所有指标必须恰好出现一次，顺序不限。遗漏、重复、未知 ID 或 envelope 无效会使全部维度按稳定原因记为无效；单个分数／理由不合法只影响对应指标。分数仍为 1～5 的整数，越高越好。聚合与 `require-complete` 逐指标执行。Provider 失败、重试、取消、并发、预算和 usage 属于整次调用，不按返回的分数重复计算。无效读数是测量证据，不作为 provider 失败触发重试。

底层迁移：`CreateRubricJudgeKitInput.metricId` 和 `RubricJudgeEvaluatorDefinitionBuilderInput.metricId` 改为 `metricIds`；`RubricJudgeKit.metricDefinition` 改为 `metricDefinitions`；`criterionPointer` 与 binding `criterion` 改为 `criteriaPointer` 和 `criteria`。`createRubricJudgeCriterion` 新增必填 `metricId`。`kit.createEvaluationContext` 接收 criteria 数组；`createRubricJudgeEvaluationContext` 的 entry 使用 `criteria: [...]`。Context schema 描述这一非空、逐项带版本的数组。

Evaluator runtime 版本为 `2.0.0`；显式版本约束也需更新。只有 `applicableSampleIds` 不重叠的 Evaluator 才能共用测量坐标；省略作用域表示全部样本，重叠坐标仍会被拒绝。Core wire schema 与 observation 身份派生保持不变。

实现、instrument、context 和 evidence identity 升为 v2；prompt template 为 `v6-multi-cot-toolargs-fmt[-len]`。已有 Core artifact 仍可读取，但新运行拒绝旧 instrument 配置。基准和候选必须使用同一 v2 声明重跑；量表仍为 1～5 不代表测量可比。产品 CLI 样本文件保留已有 rubric 格式及权重，编译时按实际维度集合分组，缺少的维度不会被补出。联合评分可能使维度错误相关；调用更少不保证分数等价或耗时同比下降。
...
新契约统一替换单指标 API，不接受旧形式。将 `metric` 改为 `metrics: [metric]`，将 `schemas.value` 改为 `schemas.values: { [metric.metricId]: parser }`，把原 score／missing／invalid 结果包装成 `{ resultKind: 'completed', results: [{ metricId, ...result }] }`，并将 `usage` 移到外层。调用级 `failed` 结果形态不变。`createFormulaEvaluator()` 的配置 API 不变，返回的 evaluator 改为同一多指标契约，包含一个结果。

Facade identity 升为 `omk.eval-runtime.custom-evaluator/v2`；`checkRuntime({ runtimeKind: 'evaluator', ... })` 使用 `omk.runtime-check.custom-evaluator/v2`。一致性探针将标量 `score.expectedValue`、`missing.expectedReasonCode`、`invalid.expectedReasonCode` 改为按所有指标 ID 提供的 `expectedValues`／`expectedReasonCodes` 映射。

既有 Core 落盘记录仍可读取，无需迁移存储 Schema；新 facade 指纹阻止复用旧 Custom Evaluator 缓存。把独立调用合并成一次会改变 evaluator identity 与测量工具，因此基线和候选版本都应使用同一新声明重新测量，指标名称相同不代表可比。63 条样本、一个版本、一次 trial，将五个指标合并后，重试和缓存命中前的计划评分调用从 315 次降到 63 次；这说明调用次数减少，不保证实际耗时缩短为五分之一。
...
`EvaluateInput` 不可整体序列化下发，也没有为它发布输入 wire schema：`variants[].execution.executor`（`execute()` / `openSession()`）、自定义 evaluator 的 `implementation`、评委的 `invoke()` 都是进程内回调。因此下发评测工作的平台宿主把契约拆成三份已发布载体：
...
版本化规则：下发契约自带 `schemaVersion`，增删字段都要 bump 该版本。所有已发布 schema 都是 strict 的，多出字段或缺少必填字段会在 preparation 阶段失败关闭，不会被改写或静默丢弃；省略的**可选**策略字段则会被物化为文档化的 canonical 默认值并封存进 Plan，生效值以 `prepared.policy` 为准。改动 rubric、metric 定义或决策阈值等于改动测量语义，必须作为新契约版本发布并标记 `BREAKING-COMPARABILITY`，旧结果与新结果在看板上分区展示。用例集的 `requires` 是宿主侧依赖预检输入，不进入 Core 测量 digest。宿主工作流、注册表模式与可比性治理见[平台宿主集成（评测中心类）](../guides/platform-host-integration)。
