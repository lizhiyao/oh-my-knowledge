...
| `createFormulaEvaluator` | 从 calculator ID 和字段映射创建声明式固定公式评分器。 |
| `FORMULA_CALCULATOR_IDS` | 内置 formula calculator 标识符（`ratio`、`ranking`、`duration`、`mean`、`percentage`）。 |
