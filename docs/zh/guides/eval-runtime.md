...
| `comparison`（比较关系） | 哪个是对照组（`control`），哪个是实验组（`treatment`），比较哪些指标。 |
...
业务规则无法由内置评分器表达时，使用 `CustomEvaluator`，每个评分器声明一个或多个指标，在每个 Sample × Variant × Trial 的一次评分尝试中共同计算。例如检查禁用 ID、字段格式，或统计输出长度。

下面统计去除前后空格后的 JavaScript 字符串长度（UTF-16 码元数，部分 emoji 会占两个或更多码元），同时记录输出是否非空，并汇总实验组的平均长度。示例声明越短越好，只用于展示规则接入，不能把长度当成整体回答质量。替换 `evaluate()` 回调、指标定义与输入 schema 即可实现自己的规则：
...
下面使用一个评委模型，对每条实际输出评分两次，再取平均，并汇总实验组的平均分。`internalGateway` 和 `judge-model` 需要替换为真实模型接入；评分会产生额外的模型调用。正式评测前应明确各分档的含义，并用人工标注样例校准标准。
