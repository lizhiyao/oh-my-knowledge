...
完成 `omk observe ingest` 后，启动 `omk studio`，进入「对话任务」即可按「对话(Thread) → 任务(Turn)」浏览已经观测的日志，再选择一次任务进入「任务轨迹」。当前列表只覆盖已经进入 `observe` 报告的对话，不代表本机全部 AI 对话。
...
- **语义轨迹**：把 Trace IR 投影为四条泳道，帮助人理解任务经过；
