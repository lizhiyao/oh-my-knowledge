...
左侧分为「项目」和「最近对话」，支持搜索和仅查看进行中的对话。设置与帮助固定在左下角。打开会话后按时间顺序连续阅读用户与助手的完整消息，默认定位到最新内容；向上滚动自动加载更早对话，也可点击「加载更早的对话」。读取历史时保持当前位置，新内容到来时提示「有新消息」，点击后回到最新位置。工具调用和原始依据按需进入「执行详情」，会话详情可直接发起知识提炼。
