...
`shared` 是跨领域叶子，只依赖自身。`eval-core` 是 Node 侧的确定性测量内核：宿主无关指它不依赖 CLI、文件系统、网络、环境变量、具体执行器与 Studio，Node 运行时本身（含 `node:crypto` 的规范化摘要）属于既定依赖，不做哈希注入化。`eval-runtime` 是服务宿主运行时层，其中每个模块都能从某个已发布入口到达——这条由 `test/architecture/production-reachability.test.ts` 机器钉住，不是散文承诺。canonical façade 将普通 `evaluate()` 输入编译为既有 Core contract，foundation 装配显式 port 与 Core 内建能力。它还持有两块宿主直接调用的已发布面：Judge／Evaluator 实现（`judges/`、`evaluators/`、`custom-evaluator.ts`）与宿主自证面（`conformance/`：`checkExecutor`、`checkRuntime`、`run*Conformance`，以及 `debugEvaluator`），宿主用它在上线前证明自己的 adapter 符合契约。产品 workflow 留在 `eval-workflows`；文件系统、目录、持久化、provider Runtime 与 UI 都在 Core 外由宿主装配。
...
下表说明各领域的边界；后续结构调整仍须逐项验证，而不是把所有领域排成同一条流水线。
...
输入 Schema 校验归属 `inputs/schemas`，执行器选择归属 `hosts/input-resolution`，API 共用输入策略归属 `hosts/adapters/shared`，供应商映射归属各自 adapter。样本加载、CLI 准入与可独立调用的 Runtime 边界保护不同入口；这些校验不能据此跳过，也不应将宿主行为移入 Core。

### Provider 调用机制与测量适配

普通 `ExecutorFn` 服务于生成、评委与辅助分析；测量适配器对执行能力、来源身份和失败证据作更严格的声明。下面按当前实现区分可共用机制与必须保留的策略，不能用普通调用接口限制 Runtime 能力。

| 路径 | 已共用或本轮提取的机制 | 保留的契约差异 |
|---|---|---|
| Codex CLI | `executors/openai/codex/cli-arguments.ts` 构造参数；共用子进程、协议事件规范化和 Trace 解析 | 普通调用选只读沙箱；测量调用显式选择沙箱、effort、严格配置及环境继承策略，并绑定可执行文件身份 |
| Claude CLI | 共用子进程、协议规范化、Trace 与 mock 基础设施 | 普通调用传 prompt 参数和可选 system 文本；测量调用使用独立的输入投影、配置来源限制及资源文件，不能直接共用整段参数模板 |
