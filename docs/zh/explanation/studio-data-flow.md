...
- **Next.js 宿主（默认 `omk studio`）**：`next-server.ts` 拦截 `/observe*`、`/measure*`、`/knowledge*` 的 GET 请求，**每请求恰好装载一次**页面模型（`loadObservePage` / `loadKnowledgePage` / `catalog.list|get`），存入经 `Symbol.for` 桥接的 AsyncLocalStorage（CLI 模块图与 Next 产物解析到同一个 store），随后渲染。服务端组件经 `web/catalog.tsx` 读取快照；store 缺失是装配错误（`studio_context_missing` → 500），不是数据源失败（→ 503）。
- **独立 report-server 宿主（DSH 插件、CLI 评测预览）**：`request-handler.ts` 每请求解析一次目录选择，经声明式路由表（`routes/router.ts`）分发到 JSON API 与 HTML 渲染器。
