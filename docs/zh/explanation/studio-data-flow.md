...
- **Next.js 宿主（`omk studio`、DSH 插件 `/omk observe`、CLI 评测预览）**：`next-server.ts` 拦截**全部 Studio 页面地址**——`/observe` 与 `/observe/conversations/*`、`/observe/inbox`、观测健康四页（`/observe/health`、`/observe/health/:reportId`、`/observe/health-diff`、`/observe/skill-trend/:skill`）、`/measure*`、`/knowledge` 与 `/knowledge/skills/*`、`/knowledge/managed*`、`/agents`，外加 `/_next/*` 静态资源——并**每请求恰好装载一次**页面模型：经 `http/pages/` 的装载器（`loadObservePage` / `loadInboxPage` / `loadHealthPage` / `loadKnowledgePage` / `loadManagedPage` / `loadMeasurePage` / `loadAgentsPage`）。这些装载器只服务本宿主：它们在 Next 开始流式响应前定下 400/404/503 契约，「有路由但记录不存在」因此一律是宿主的纯文本 404（#902 §三）。`/agents` 是刻意的例外：盘上缺 `omk agents` 报告或报告读不动都是页面能呈现的事实，因此它把「还没跑过命令」与「报告读不动」分开写给用户，而不是起一个 503。地址根本没有路由时不落进 Next，由 HTTP 适配器给纯文本 404；带壳的 `not-found` 文档只是 Next 客户端路由失配时的边界，段内的 `notFound()` 也担不起服务端契约，因为根 `loading.tsx` 会先刷出壳层，状态码因此停在 200。模型存入经 `Symbol.for` 桥接的 AsyncLocalStorage（CLI 模块图与 Next 产物解析到同一个 store），随后渲染。服务端组件经 `web/catalog.tsx` 读取快照；store 缺失是装配错误（`studio_context_missing` → 500），不是数据源失败（→ 503）。拦截集合跟随宿主的开关收缩：`observationInbox: false` 摘掉收件箱页面与 API，`studioPages: false` 摘掉整个观测／知识页面组，被摘掉的路径不再拦截，落回 HTTP 适配器得到 404。CLI 评测预览设置 `studioPages: false`：它只提供 `/measure`，并且同一个开关让壳层不渲染一级导航，而不是给出指向 404 的链接。
- **独立 report-server 宿主**：`request-handler.ts` 每请求解析一次目录选择，经声明式路由表（`routes/router.ts`）分发到 JSON API、SSE、`/health` 与纯文本状态文档。它不渲染任何页面 HTML——最后两页只读报告迁到 Next 后，共享 HTML 渲染层已删除，因此即使 Next 宿主裁掉了页面组，这里也不会有页面地址被服务，那条路径直接 404。Next 宿主包裹的就是这个 handler，未被拦截的路径——包括评测 JSON 投影 `/api/reports`，以及 `GET /`（挂了观测路由组的宿主 302 到 `/observe`，其余给 404）——都由它服务。`studioPages: false` 同时抑制 observations 目录的创建副作用，因此只挂评测页的宿主不会在用户项目里落盘。
