...
这条路径不要求预先收集日志。已有真实任务时，也可以从 Studio 观测、知识提炼与复核开始，再把需要采纳的内容落实到载体与用例。两条路径共享同一原则：决定要能回到来源和评测契约核对。
...
- **observe（观测）**：真实使用反馈闭环。它从真实 session trace 里发现现有契约没有覆盖的知识缺口，反哺下一轮用例，但不能替代受控 eval，也不应该成为新用户第一步必须理解的入口。
