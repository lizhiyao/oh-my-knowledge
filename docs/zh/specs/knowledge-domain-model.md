...
Studio 从观测会话选择任务和消息，复用同一个 ConversationCatalog 与 Codex 证据投影；日志文件导入作为补充。服务端根据会话身份解析来源，按预览版本校验所选消息，固定快照后才允许确认模型调用，不自动加入未选消息。快照与提炼记录保存可选的会话、任务、标题和工作目录来源，候选可返回任务原文，会话可查询所选知识目录内的提炼记录与维护选择。删除来源快照后仍保留提炼关联，原始证据是否可读另行显示。原有不含会话来源的快照与记录仍可读取；不会猜测其会话关联。

全局设置参见 [配置范围](../reference/settings.md)。Studio 默认目录为 `$OMK_HOME/knowledge`，单次显式目录优先于全局偏好；浏览器不再保存独立的长期目录。


Studio 在会话详情原地确认并发起提炼，默认覆盖当前会话的对话消息；单轮详情默认只包含这一轮。逐条选择作为折叠的可选操作，配置继承全局偏好。确认前仅本地读取，确认后才固定快照并调用模型；进度和结果留在当前页，候选进入知识页核对。整段会话来源不含 turnId，已有单轮来源保持可读；超限或不完整来源会拒绝，不静默截断。
