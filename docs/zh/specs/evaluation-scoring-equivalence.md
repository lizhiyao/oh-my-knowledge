...
第四个派生由 [#496](https://github.com/lizhiyao/oh-my-knowledge/issues/496) 引入的宿主 Analysis 节点 `omk.assertion-layer-table/v2` 实现。其密封参数把每个唯一准则和布尔 Metric 显式映射到 `fact`、`behavior` 或 `excluded-mixed-layer`，并附带有限正权重；节点绝不从 assertion 名称、Evaluator ID 或证据推断分类。每个目标／样本／试验行保留完整准则状态和 Core 派生的抽样单元血缘。`criterion-not-applicable` 是结构性的，不计入 assertion 评分覆盖；Analysis Bundle v2 仍在 `planned` 中保留这个矩形坐标，将其放入独立的 `notApplicable` 桶，并通过 `notApplicableRows` 认证行身份和原因。因此它不会降低证据完整度；其它所有未观测状态仍会降低覆盖，但不会变成 `false` 或分数零。表验证器会重算加权分数和覆盖，并强制执行规范顺序、全局唯一来源血缘、内容派生的组身份，以及在同一样本的目标和试验之间保持一致的准则设计；跨样本共用的 Metric 保持相同准则、层和权重。Runtime 指纹绑定这些语义和两个 schema 身份。生产 CLI 通过已注册的 Core AnalysisGraph 消费该节点。

**BREAKING-COMPARABILITY — 按样本范围汇总断言：** v2 断言表与参数 Schema 封存每个 Evaluator Metric 适用的样本 ID。节点（implementation version 3.0.0）要求每个测量单元恰好包含每个适用 Metric 的一条真实行，缺行或范围外行均拒绝；不会给该题从未规划的检查补造行。这修复了多样本结构化检查单项评分成功、旧矩形汇总却失败的问题。加权计分公式、显式缺失证据规则与 Core 契约不变。新 Schema identity 同步改变下游指纹，应重新运行比较，不复用旧分析绑定。Beta 直接替换 v1 断言表／参数契约，不保留兼容读取器。
