...
Evaluation coordinate 使用 canonical `(targetId, sampleId, trialIndex, evaluatorId)` 顺序。`evaluationId`、attempt ID 和 observation ID 使用 domain-separated digest 派生。每条 active EvaluationRecord 都绑定准确的 canonical ExecutionRecord digest 和已解析的 Evaluator RuntimeIdentity。cache key 还绑定 EvaluationPlan、物化后的输入、source record 与 effective source trust，因此 Gold、Evaluator identity、binding、execution evidence 或 source trust ceiling 任一变化都不能静默复用旧评分。cache replay 必须先完整校验 record schema、retry identity、有序 metric contract、scale、source digest 和 runtime identity；replay provenance 不得提升 source trust。
