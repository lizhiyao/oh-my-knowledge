...
Series start 路径返回 `{ events, result }`。有界、单消费者 event stream 只承载生命周期通知；即使没有实时消费者，result 也必须 settle。Event 使用注入 clock 与单 Run 确定性 sequence，只携带 identity、terminal state 和脱敏 reason code，不携带 member evidence 或 Runtime error。可选 EventWriter 只有一套固定的观测语义：write 保序并施加背压；第一次 writer 失败后停止后续 write；writer 失败不能改变 Series result。未来若要让外部交付失败终止 Series，必须先把该语义封存为 Series policy，不能增加 start-time 开关。
