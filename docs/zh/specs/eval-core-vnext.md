...
同时区间 family 使用独立、版本化的 correction contract。`simultaneous-intervals.bonferroni/v1` 至少消费两个已完成的 percentile-interval result；每个 result 的 producer 必须精确绑定一个彼此不同的 Comparison contrast 与 Metric。标称 family 置信度为 `1 - alpha`、预注册 family size 为 `g` 时，每个 member estimator 封存 marginal alpha `alpha / g` 与相同的重采样次数。correction 输出 canonical table，包含准确的上游区间值、family 与 marginal 置信度目标、family size 和重采样次数；Core 会在在线执行与传输后 Bundle 校验时再次核对这些值。Bonferroni 同时覆盖仍依赖每个边际区间过程达到其标称覆盖率；percentile Bootstrap 不构成无条件的有限样本保证。correction 绝不派生或伪造 p-value。family member、cohort、置信度和重采样参数都必须在 Target 执行前封存；基于结果动态选择 member 必须使用不同的 exploratory analysis identity。

DecisionPolicy 的每个 comparison family member 都声明 `(comparisonId, treatmentTargetId, metricId, analysisResultId)`。Core 接受两种显式且不能混用的 ownership 形态。Estimator-owned 形态中，每个 member 都绑定同一 `comparisonFamilyResultId`，由该 estimator 在内部封存完整 family。Composed-result 形态中，每个 member 拥有独立 result，其 producer 精确且仅消费该 member 的 Metric 与 Comparison selector；`comparisonFamilyResultId` 则指向一份与 member 分离的权威 table，该 table 必须精确消费全部 child result。通用 raw-hypothesis family 使用不带 `comparisonFamilyResultId` 的 composed 形态：它还要声明 canonical `hypothesisId`，correction node 精确消费 member result，DecisionPolicy 消费唯一 corrected result。Raw-hypothesis family 不能声明权威 family 字段，以免绕过 raw-p lineage 核对。超过一个 member 时始终必须声明 `multipleComparisonPolicyId`，已解析 DecisionPolicy capability 也必须支持同一标准。Core 绝不伪造 p-value。空或 singleton family 不能伪装成多重比较。Decision 只能收到带声明 result identity 和可选 hypothesis identity 的投影 contrast，不能看到所属 Comparison 中无关的 treatment 或 Metric。通用 correction table 必须匹配 canonical hypothesis ID、family size 与 raw p-value；simultaneous-family table 则必须通过版本化输出 schema、精确 child lineage 与 DecisionPolicy binding 校验，之后才能产生 verdict。

内建 `progress/v1` 作为历史点估计策略冻结保留；当前 `progress/v2` 只选择 singleton contrast 绑定的区间 result，没有 family 时只接受唯一声明的区间 result，并且只有完整置信区间排除配置的 threshold 加 equivalence band 后才给出方向性 verdict。输入有歧义或不是区间时返回 not-decided，并且不声称支持 multiple-comparison。内建 `release-family/v1` 精确消费一份 Bonferroni 同时区间 table，并要求为每个 member 显式声明、按 canonical 顺序排列的 criterion。Criterion 使用原始 treatment-minus-control effect 单位：整个区间落在声明的 inclusive minimum／maximum boundary 内才证明 acceptable；整个区间严格落在某条 boundary 外才证明 unacceptable；其余情况均为 uncertain。当前唯一支持的 `all` 规则下，全部 member acceptable 才返回 `RELEASE`，任一 member unacceptable 即返回 `BLOCK`，其余返回 not-decided。该策略不应用 Metric direction、weight、normalization、composite score、p-value、`any` 或 k-of-n 语义。
...
`prepareEvaluationPlan()` 是 Comparability 接受的进程内 `SealedRunPlan` capability 唯一签发入口。注册与验证该 capability 的 authority 位于 Core-private contract module；它不进入 contracts barrel，并由 package export map 阻断。Compiler 与 Core consumer 通过相对导入使用它，不把签发能力暴露为包解析可达的 consumer 入口。capability contract 与 `RunPlan` 相邻，也消除了 `contracts ↔ internal` 依赖环。RunPlan 字段仍可序列化为 JSON 供审计，但 clone 或 transported document 不具备比较权限；`assessComparability()` 再次消费前必须经过 Runtime resolution 重新 prepare。这样可以阻止调用方保留旧 digest 与 authenticated stage source，却替换 Target、instrument、sampling、Analysis 或 Decision projection。
...
Evaluation coordinate 使用 canonical `(targetId, sampleId, trialIndex, evaluatorId)` 顺序，并绑定 Evaluator 显式的 instrument、ensemble-member、replicate-group 与 replicate-index identity。`evaluationId`、attempt ID 和 observation ID 使用 domain-separated digest 派生。每条 active EvaluationRecord 都绑定准确的 canonical ExecutionRecord digest、评委测量坐标和已解析的 Evaluator RuntimeIdentity。v2 cache key 绑定 EvaluationPlan、evaluation ID、已解析的 Evaluator RuntimeIdentity、effective source trust，以及评委实际可见的完整物化 binding closure。因此 Gold、Evaluator identity、binding、评委可见的 execution evidence 或 source trust ceiling 任一变化都会失效旧评分；未进入该 closure 的 source-record 字段，例如 output-only Evaluator 不可见的 wall-clock timing，则不会制造伪 miss。cache replay 必须先完整校验缓存的 native record schema、retry identity、有序 metric contract、scale、runtime identity、attempt 到 record 的确定性 usage 聚合与 provider-cost eligibility。返回的 EvaluationRecord 会重新绑定当前 canonical ExecutionRecord digest；历史缓存的 EvaluationRecord digest 则独立保留在 replay provenance 与 cache receipt 中。replay 不得提升 source trust；transported Bundle 若没有独立观测到的 receipt，其 cache 及相关预算验证必须保持 `indeterminate`。
...
`test/eval-core/conformance/` 建立确定性、宿主无关的共享 harness。纯函数、RAG top-K
...
`startEvaluationSeries()` 先检查 member slot 唯一性、可选 expected Run identity，并在 preregistered 模式下校验成员 Run 精确的执行前 Series binding；没有匹配 binding 的 post-hoc Run 会 fail closed。`runEvaluationSeries()` 只是同一 start 路径的 Promise 便捷代理，不形成第二套实现。随后 Core 按显式 ComparabilityPolicy 从 anchor Run 对每个候选执行比较，并把每个非 anchor 成员的完整认证 ComparabilityAssessment 持久化到 SeriesAnalysisBundle，使 design incompatible、evidence conditional 与 identity change 保持可审计，而不是塌缩成一个计数。design incompatible 永不接纳。evidence conditional 只有在预注册 policy 允许 `conditional` 时才能进入分析；identity change 不能伪装成 missing member。missing、partial、cancelled、budget-exhausted 和 failed Run 全部进入 `SeriesMemberCoverage`，不得静默丢弃或折算为零。

Series start 路径返回 `{ events, result }`。有界、单消费者 event stream 只承载生命周期通知；即使没有实时消费者，result 也必须 settle。Event 使用注入 clock 与单 Run 确定性 sequence，只携带 identity、terminal state 和脱敏 reason code，不携带 member evidence 或 Runtime error。Event 生成或交付失败不能改写 artifact 或 terminal result。可选 EventWriter 只有一套固定的观测语义：write 保序并施加背压；第一次 writer 失败后停止后续 write。未来若要让外部交付失败终止 Series，必须先把该语义封存为 Series policy，不能增加 start-time 开关。

取消统一使用一个 `AbortSignal`，并在每次外部 Runtime 调用前后检查。terminal result 明确区分 `completed`、`cancelled` 与 infrastructure `failed`。取消结果只会携带在取消前已经完整物化的 Analysis bundle 或 Decision result，不会用部分 node work 伪造 completed Series report。每个 Series Analysis node 与 Decision policy 都打开 run-scoped session，并精确 dispose 一次；取消会等待 in-flight call settle 后再释放资源，一个 Run 不得关闭另一个 Run 的 session，dispose 失败优先于 cancellation，成为 infrastructure failure。Series start 同时捕获 Runtime map、identity、output schema、validator、clock method 与 `openRun` 函数，宿主后续 mutation 不能改变 active run。
