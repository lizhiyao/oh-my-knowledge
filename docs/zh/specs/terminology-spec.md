...
**「候选知识」不是第三个对象层，而是知识内容所处的状态。** `knowledge-domain-model.md` 写明知识改动经历提议、复核、形成候选、关联评测、采纳或拒绝。因此界面可以写「候选知识」，但它必须被读成"尚未复核的那条知识"：不得与内容层并列成一种新对象，也不得被术语门禁当成与内容互斥的类别。同理不封闭这一侧的界面词汇——修订、来源证据、知识缺口等既有概念按各自定义出现。
