...
**绝对不进 judge prompt**（`buildJudgePrompt(prompt, rubric, output, traceSummary)` signature 不含 sample 对象，且有 `test/eval-workflows/instruments/judge-prompt-isolation.test.ts` 防御回归）。**绝对不影响 verdict 算法**。这是构造效度保护的硬要求 —— judge 看到 "construct: necessity" 等于知道试题答案。
...
- 不进 `buildJudgePrompt` signature（`test/eval-workflows/instruments/judge-prompt-isolation.test.ts` 防御回归）
