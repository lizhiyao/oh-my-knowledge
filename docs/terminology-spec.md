...
具体证据(v0.22 之前):N=20 WCC 评测 baseline 平均 13 次 `Skill` 工具调用 + 输出 13/20 含 `@alipay/wealth-chart-components` 私有 import,verdict NOISE Δ=-0.0006 — 看起来 skill 没用,实际是 baseline 跟 treatment 看到的是**同一份内容**,Δ 接近 0 是必然结果。三堵之后 verdict PROGRESS Δ=+1.03 CI [0.50, 1.51],真实差异显现。
...
**为什么 cwd 这条 channel 单独列出**:N=20 WCC E2E 验证发现 — 仅堵 SDK 两条 channel(`skills:[]` + `disallowedTools:['Skill']`)后,baseline 的 `Skill` 工具调用确实从 13 次降到 0 次,但 `@alipay/wealth-chart-components` 私有包名仍在 13/20 个 baseline 输出里。trace 显示 baseline 用了 63 次 `Read` 工具直接读 `~/.claude/skills/wealth-chart-components-skill/SKILL.md`,完全绕过 SDK 隔离。根因:omk 默认 `baseline.cwd === null` → SDK fallback 到 `process.cwd()` = 用户评测工作目录,那里通常有 `skills/<name>/` symlink 给 treatment 用,baseline 用 plain `Glob "**/*"` + `Read` 顺 symlink 直读。修法是 baseline 默认 cwd 切到 `~/.oh-my-knowledge/isolated-cwd/`(stable 空目录,cache 一致)。补完后 baseline `@alipay` 输出降到 2/20(剩下 2 处是 prompt 自含 — `wcc-001` / `wcc-018` 的题面里就有包名,属于用例设计而非 contamination),verdict NOISE → PROGRESS Δ=+1.03 CI [0.50, 1.51] 不跨 0。**用户显式给 baseline 设 cwd 时不动**(显式 cwd = 用户负责该目录干净)。

注:isolated-cwd 不是 sandbox,baseline 仍可 Read 任意 absolute path。但模型不会主动猜 `/Users/.../skills/...` 这种用户私有路径(没 system prompt 暗示),trace 里也没见到这种行为。如果你的评测场景里 baseline 会被 prompt 引导去读绝对路径,那就需要再加层 sandbox 保护(out-of-scope)。

### 5. 跨版本可比性

**v0.22 是 breaking-comparability**(等同 judge prompt bump 一档):
- pre-v0.22 报告(无 `meta.skillIsolation` 字段)与 v0.22 报告**不可跨版本对比 verdict / Δ**——前者 baseline 被 auto-discovery 污染,后者干净。
- `--resume` 老报告时,如果 isolation 状态不一致,stderr warn 不阻塞;用户决定是否 `--no-cache` 强制重跑。
- cache key 升级到 `v2:` prefix,旧 cache 一次性失效(避免 strict / non-strict 切换误命中)。
