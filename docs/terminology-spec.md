...
omk 跑 baseline-vs-skill 评测时,baseline variant 默认通过 Claude Agent SDK 的 skill auto-discovery 拿到了 `~/.claude/skills/` 里的所有 skill。如果用户的 home dir 已装了被测 skill(或同领域的等价 skill),baseline 实际不是"裸模型"——**construct invalidity**。

具体证据(v0.22 之前):N=20 WCC 评测 baseline 平均 13 次 `Skill` 工具调用 + 输出含 `@alipay/wealth-chart-components` 私有 import,verdict NOISE,但实际是污染。
...
### 6. degraded mode(executor 兼容)

| Executor | undefined | `[]` | `[name]` |
|---|---|---|---|
| `claude-sdk` | 默认全发现 | skills:[] + disallowedTools:[Skill] | skills:[name] |
| `claude-cli` | 默认 | `--disable-slash-commands` + warn | **throw**(用户改 sdk) |
| `script` | 默认 | stderr warn,不阻塞(无效) | stderr warn,不阻塞(无效) |

claude-cli executor 没有 `--skills` flag(只有 `--disable-slash-commands` / `--bare`),只能粗粒度 disable;白名单需求强制走 claude-sdk。
