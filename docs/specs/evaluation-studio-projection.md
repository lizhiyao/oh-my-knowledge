...
The human-facing `/measure` list and detail pages are React server components under `web/app/measure/**`. The formatting layer they consume, `application/core-run-format.ts`, is presentation-free: it turns the two versioned views into ordered fact fragments, so a list presents run, evidence, and conclusion status as three separate axes, and a detail presents plan identity, stage coverage and budgets, safe records and numeric observations, Analysis, Decision, and the complete five-document lineage. Neither view infers an overall quality state from scores.
