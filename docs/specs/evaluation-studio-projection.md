...
## 4. Presentation and route adapter

The human-facing `/measure` list and detail pages are React server components under `web/app/measure/**`. The formatting layer they consume, `application/measure/core-run-format.ts`, is presentation-free: it turns the two versioned views into ordered fact fragments, so a list presents run, evidence, and conclusion status as three separate axes, and a detail presents plan identity, stage coverage and budgets, safe records and numeric observations, Analysis, Decision, and the complete five-document lineage. Neither view infers an overall quality state from scores.

Every projected value is escaped by React's text interpolation — the standalone HTML renderer and its shell are gone, so no Studio page is built by string concatenation — and fields outside the allow-list never reach the page. Tables pair a visible block name with scoped column headers, status groups have accessible labels, and the Chinese and English views carry the same facts: only labels are translated, identifiers, digests, statuses, and reason codes stay verbatim. `web/components/measure/**` is the only renderer; the previous standalone HTML renderer and its `CoreStudioRenderRoutes` seam are removed.

`createCoreStudioRouteHandler()` is a pure HTTP-shaped adapter over `CoreStudioCatalog` that serves only the machine-readable resources. It returns an immutable response envelope instead of depending on Node request/response objects, so the production host mounts it without giving the catalog server authority. A single caller-provided `apiBasePath` owns its whole subtree: paths outside it return `undefined`, the base path lists cards, one decodable segment resolves a detail, and empty, nested, or undecodable identifiers return stable 404 responses. Unsupported methods return 405, and source failures return a redacted `core_studio_source_unavailable` response without exception text or filesystem paths.
...
Studio uses Next.js App Router, TypeScript, and Ant Design as its application stack. The `/measure` list and detail pages are React components on every host, including the CLI evaluation preview, which boots the Next host with the sibling page groups trimmed. Observe conversation lists, details, and live task trajectories also use React, as do the observe-health pages, the observation inbox, Knowledge lists and details, and the managed decision history; they share the same StudioShell and Ant Design components. No Studio page is rendered by the retired HTML renderer any more, so a page has exactly one implementation with no fallback.

The Node listener owns Next's preparation and shutdown. Server components receive the existing `CoreStudioCatalog` through request-scoped context, so simultaneous Studio instances cannot read each other's catalogs. The catalog is resolved before HTML streaming to preserve 404/503 responses. The JSON API keeps its contract; the standalone Core rendering exports are removed together with the HTML measure renderer, and `/measure` has no second implementation to drift from. This migration changes neither Core artifacts nor measurement semantics.
