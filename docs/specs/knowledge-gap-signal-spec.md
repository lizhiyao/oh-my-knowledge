...
omk already has the cross-evaluation time-series table built by `querySkillTrend` in `src/studio/application/knowledge/knowledge-reports.ts` and rendered by the `/observe/skill-trend/:skill` page. This spec requires a new column in that table:
