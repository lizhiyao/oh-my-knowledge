...
What ships in the MVP (`omk rollback <name>`): revoking the acceptance of the **current** content. Rolling back a not-promoted version exits non-zero (nothing to revoke); an already-rolled-back version is an idempotent no-op; and `promote → rollback → promote` restores `promoted` (latest wins). Restoring an *older promoted version's content* to the source (a true file restore) is **out of scope — delegated to git**. omk does not store version bytes or run a version store (that would be the "better file copier" §1 disclaims), and it does not execute the restore against the user's working tree (intrusive, and git's lane). What omk adds here is in-identity and small (#236): a per-version git coordinate (SHA) recorded on the evidence as a pointer, so `list` / Studio can show the evidence-backed version history and, for a git source, the exact `git checkout` to return to a chosen version. The coordinate is recorded only when it actually pins the measured bytes — a **local git source measured on a clean working tree**; a remote source's restore is a re-install of its record-level pinned SHA (not a cwd `git checkout`), and a dirty tree has no commit that reproduces the measured content (so freshly-`evolve`d, uncommitted output records no coordinate). A non-git, in-place-edited source has no coordinate to restore from — the honest answer is to version-control the skill with git; omk will not reinvent that.
