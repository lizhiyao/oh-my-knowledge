...
| **Samples** (coverage) | `currentSampleSetHash !== evidence.sampleCoverage.hash`, assessed **only** when a current sample set resolves (an `eval.yaml` / sample set in context) | none — derived `sampleDrift` marker | **Warn, forceable with `--reason`** — same artifact, the case set moved; the human owns the coverage gap or re-runs `eval` to refresh. |
