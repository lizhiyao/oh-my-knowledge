...
- **Decided (#214):** the artifact content hash is unified so evidence can bind, keyed on *what the executor actually measures*. For local directory-skills `eval` records the same whole-tree `hashArtifactSource` as `install` (report `schemaVersion >= 2`) — `cwd` is anchored to the skill root so `references/` assets are real runtime input — so `evidence.contentHash === record.contentHash` lives in one space and the fingerprint is no longer blind to asset changes (the executor cache key carries the same hash, so an asset edit busts the cache). File-skills (local or git) hash the single `.md` bytes and also bind. **Git directory-skills are the exception:** git eval injects only `SKILL.md` (`cwd`/`skillDir` null, `references/` never exposed to the executor), so their eval fingerprint covers `SKILL.md` only and does *not* match the install whole-tree record. Binding git directory-skill evidence is deferred until git eval materializes the tree and exposes it to the executor (faithful execution). Reports with `schemaVersion < 2` carry the legacy SKILL.md-text hash and are treated as incomparable by drift / lineage consumers (re-run `eval`). Actually writing eval→managed evidence remains future work.
