...
The assignment redesign was an intentional `BREAKING-SCHEMA` cutover: ExecutionPlan requires v2 and EvaluationPlan remains v1. The later explicit Analysis-selection cutover advances EvaluationDefinition, AnalysisPlan, and RunPlan to v3 because a node can now seal `targetFilter.includeTargetIds`; no earlier-version reader, migration adapter, or compatibility diagnostic is provided. `stratumKey` lives on `AssignmentDesign`, and custom Analysis Runtime capabilities declare their supported `assignmentKinds`. Built-in assignment-aware Analysis Runtime identities are versioned when their capability fingerprints change. Pre-cutover persisted Plans must be regenerated from their source definitions before use.
...
Simultaneous interval families use a separate, versioned correction contract. `simultaneous-intervals.bonferroni/v1` consumes at least two completed percentile-interval results whose producers each bind exactly one distinct Comparison contrast and Metric. For nominal family confidence `1 - alpha` and preregistered family size `g`, every member estimator seals marginal alpha `alpha / g` and the same resample count. The correction emits a canonical table containing the exact upstream interval values, family and marginal confidence targets, family size, and resample count. Core checks those values again during live execution and transported Bundle validation. Bonferroni simultaneous coverage still depends on each marginal interval procedure having its stated coverage; percentile Bootstrap does not create an unconditional finite-sample guarantee. The correction never derives or fabricates p-values. Family membership, cohort, confidence, and resampling parameters are sealed before Target execution; result-driven member selection requires a different, exploratory analysis identity.

Every DecisionPolicy comparison-family member declares `(comparisonId, treatmentTargetId, metricId, analysisResultId)`. Core admits two explicit, non-mixable ownership shapes. In the estimator-owned shape, every member binds the same `comparisonFamilyResultId`; that estimator seals the complete family internally. In the composed-result shape, each member owns a distinct result whose producer consumes exactly that member's Metric and Comparison selector, while `comparisonFamilyResultId` names a separate authoritative table produced from exactly those child results. A generic raw-hypothesis family uses the composed shape without `comparisonFamilyResultId`: it additionally assigns canonical `hypothesisId` values, its correction node consumes exactly the member results, and the DecisionPolicy consumes the unique corrected result. Raw-hypothesis families cannot claim the authoritative-family field, preventing them from bypassing raw-p lineage checks. A family with more than one member always declares `multipleComparisonPolicyId`, and the resolved DecisionPolicy capability must admit the same standard. Core never manufactures p-values. Empty and singleton families cannot claim multiple-comparison correction. Decision receives only projected contrasts carrying the declared result and optional hypothesis identities, never the enclosing Comparison's unrelated treatments or Metrics. Generic correction tables must contain the same canonical hypothesis IDs, family size, and raw p-values; simultaneous-family tables are validated against their versioned output schema, exact child lineage, and DecisionPolicy binding before a verdict can be produced.

The built-in `progress/v1` policy is frozen as the historical point-estimate policy. The current `progress/v2` policy selects the interval result bound by a singleton contrast, or the only declared interval result when no family exists, and returns a directional verdict only when the complete confidence interval excludes the configured threshold plus equivalence band. It returns not-decided for ambiguous or non-interval input and does not claim multiple-comparison support. The built-in `release-family/v1` policy consumes exactly one Bonferroni simultaneous-interval table and an explicit, canonically ordered criterion for every member. Criteria use the raw treatment-minus-control effect units: a member is proven acceptable only when its whole interval is within every declared inclusive minimum／maximum boundary, proven unacceptable only when its whole interval lies strictly outside a boundary, and otherwise uncertain. With the only supported `all` rule, all acceptable members produce `RELEASE`, any unacceptable member produces `BLOCK`, and the remaining cases are not-decided. The policy does not apply Metric direction, weights, normalization, composite scores, p-values, `any`, or k-of-n semantics.

The built-in reducer and estimator set remains deliberately small:

- `descriptive.mean/v1`, `descriptive.rate/v1`, and `descriptive.quantile/v1`;
- `descriptive.hierarchical-mean/v1`, `descriptive.hierarchical-rate/v1`, and `descriptive.hierarchical-quantile/v1`;
...
`prepareEvaluationPlan()` is the sole issuer of the in-process `SealedRunPlan` capability accepted by Comparability. The authority that registers and verifies this capability lives in a Core-private contract module that is omitted from the contract barrel and denied by the package export map; the Compiler and Core consumers use relative imports without exposing the issuer as a package-resolved consumer entry point. Keeping the capability contract beside `RunPlan` also prevents a `contracts ↔ internal` dependency cycle. The RunPlan fields remain JSON-serializable for audit, but a clone or transported document is not a comparison authority: it must be prepared again through Runtime resolution before `assessComparability()` can consume it. This prevents a caller from retaining old digests and authenticated stage sources while substituting different Target, instrument, sampling, Analysis, or Decision projections.
...
The active catalog publishes twenty-one JSON Schema 2020-12 roots across `schemas/eval-core/v1/` through `schemas/eval-core/v4/`. EvaluationDefinition and RunPlan are v4; ExecutionPlan and AnalysisPlan are v3; AnalysisBundle, EvaluationReport, ComparabilityAssessment, and SeriesAnalysisBundle are v2; the remaining roots are v1. Frozen superseded roots remain addressable at their original catalog identities and are protected by committed digests. TypeScript types are inferred from the active Zod 4 schemas. `yarn build:schemas` regenerates active files without rewriting historical roots, while `yarn build` checks committed output for drift and copies the catalog into the package build.
...
Evaluation coordinates use canonical `(targetId, sampleId, trialIndex, evaluatorId)` order and bind the Evaluator's explicit instrument, ensemble-member, replicate-group, and replicate-index identity. `evaluationId`, attempt IDs, and observation IDs use domain-separated digest derivation. Every active EvaluationRecord binds the exact canonical ExecutionRecord digest, evaluator measurement coordinate, and resolved Evaluator RuntimeIdentity. The v2 cache key binds the EvaluationPlan, evaluation ID, resolved Evaluator RuntimeIdentity, effective source trust, and the complete materialized binding closure visible to the Evaluator. Consequently, Gold, evaluator identity, bindings, evaluator-visible execution evidence, or the source trust ceiling cannot change without invalidating the score, while source-record fields outside that closure, such as wall-clock timing for an output-only Evaluator, do not cause false misses. Cache replay validates the full cached native record schema, retry identities, ordered metric contract, scale, runtime identity, deterministic attempt-to-record usage aggregation, and provider-cost eligibility before accepting a hit. The returned EvaluationRecord is rebound to the current canonical ExecutionRecord digest; the cached EvaluationRecord digest remains independently visible through replay provenance and the cache receipt. Replay never raises source trust, and a transported Bundle without an independently observed receipt leaves cache and related budget verification indeterminate.
...
The built-in registry provides six descriptive reducers, eight deterministic percentile-bootstrap estimators, raw-p-value Bonferroni correction, Bonferroni simultaneous-interval aggregation, explicit exclusion MissingPolicy, and a minimal progress DecisionPolicy. Each built-in reducer and estimator seals exactly one Metric input. Bootstrap draws are domain-separated from the sealed root seed, AnalysisPlan digest, node identity, and replicate index. Repeated trials and measurement-panel replicates are first reduced within their declared sampling unit; paired contrasts are formed within complete pairing blocks before resampling; cluster bootstrap resamples whole clusters. Insufficient units or failed assumptions yield an inconclusive result and never select a fallback estimator. Built-in Runtime identities are self-reported and carry `assuranceLevel: declared`; only an independent host verifier or attestation boundary may promote executing code to verified assurance.
...
host-independent harness under `test/eval-core/conformance/`. Pure function, RAG top-K,
...
`analysisInputDigest` covers the Analysis projection. It enters AnalysisPlan, DecisionPlan, and `runContractDigest`, but not ExecutionPlan or EvaluationPlan. Changing a holdout or cohort therefore cannot perturb Target execution, evaluator cache identity, or evaluator-visible Gold. AnalysisPlan materializes the analysis samples and cohort registry, and both are passed to Analysis Runtime as sealed execution context. Each metric row carries canonical `targetId` and `cohortIds`; a node applies its sealed `targetFilter` and `cohortFilter` without parsing sample IDs, array positions, or host closures. Core independently recomputes the eligible row universe from the sealed Plan when admitting an AnalysisBundle, so a producer cannot widen, narrow, or relabel the selected population. Report and Event contracts never copy raw analysis context automatically.
...
Cancellation is cooperative through one `AbortSignal`, checked before and after every external Runtime call. The terminal result distinguishes `completed`, `cancelled`, and infrastructure `failed`. A cancelled result includes an Analysis bundle or Decision result only if that immutable artifact finished materializing before cancellation; it never fabricates a completed Series report from partial node work. Every Series Analysis node and Decision policy opens a run-scoped session and disposes it exactly once. Cancellation waits for the in-flight call to settle before disposal, one Run never closes another Run's session, and disposal failure takes precedence over cancellation as an infrastructure failure. Runtime maps, identities, output schemas, validators, clock methods, and `openRun` functions are captured at Series start so later host mutation cannot alter an active run.
