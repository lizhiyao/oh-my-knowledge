...
Judge prompts are a deliberately single-language instrument: the rubric, RAG and semantic prompts are written in Chinese and do not follow the output language resolved from `--lang` / `OMK_LANG` / saved settings / system locale. A prompt's bytes enter the host Runtime identity, so letting an ambient locale pick them would make one sealed configuration measure a different instrument on another machine, and a series comparison would silently change construct. The visible cost is stated rather than hidden: in the English interface the judge instructions stay Chinese, and a judge's `reason` text is usually Chinese. Translating the instruments would add registry prompt IDs, published instrument-schema enum values and newly frozen hashes, i.e. a versioned `BREAKING-COMPARABILITY` change; [#1043](https://github.com/lizhiyao/oh-my-knowledge/issues/1043) closed that option by maintainer decision on 2026-09-22 instead of binding instrument identity to environment state. Documentation that quotes a judge prompt shows its original Chinese bytes — a translated quote would misrepresent the instrument.
...
The fourth derivation is implemented by the host-owned `omk.assertion-layer-table/v2` node from [#496](https://github.com/lizhiyao/oh-my-knowledge/issues/496). Its sealed parameters explicitly map each unique criterion and Boolean Metric to `fact`, `behavior`, or `excluded-mixed-layer` plus a finite positive weight; the node never infers classification from assertion names, Evaluator IDs, or evidence. Every target／sample／trial row retains the complete criterion status and Core-derived sampling-unit lineage. `criterion-not-applicable` is structural and excluded from assertion scoring coverage, while Analysis Bundle v2 retains the rectangular coordinate in `planned`, classifies it in the separate `notApplicable` bucket, and authenticates its row identity and reason through `notApplicableRows`. It therefore does not degrade evidence completeness; every other non-observed status still reduces coverage without becoming `false` or score zero. The table validator recomputes weighted scores, coverage, canonical ordering, globally unique source lineage, content-derived group identity, and a criterion design that must remain identical across targets and trials for each sample; shared Metrics retain the same criterion, layer, and weight across samples. Runtime fingerprinting binds these semantics and both schema identities. The production CLI consumes this node through the registered Core AnalysisGraph.

**BREAKING-COMPARABILITY — sample-scoped assertion aggregation:** the v2 assertion table and parameter schemas seal the applicable sample IDs of each evaluator Metric. The node (implementation version 3.0.0) requires exactly one real row per applicable Metric in each unit, rejecting missing or out-of-scope rows. It does not fabricate rows for checks that were never planned for that sample. This fixes multi-sample structured checks whose individual readings succeeded while the former rectangular reducer failed. The weighted score formula, explicit missing-evidence rules, and Core contracts are unchanged. New schema identities also update downstream fingerprints; rerun comparisons rather than reuse old analysis bindings. Beta replaces the v1 assertion table/parameter contract without a compatibility reader.
