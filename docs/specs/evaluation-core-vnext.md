...
The Series start path returns `{ events, result }`. Its bounded single-consumer event stream is lifecycle notification only and never needs a live consumer for the result to settle. Events use an injected clock, deterministic per-run sequencing, and contain identities, terminal states, and sanitized reason codes rather than member evidence or Runtime errors. The optional EventWriter has one fixed observational contract: writes are ordered and backpressured, the first writer failure disables further writes, and writer failure cannot change the Series result. If external delivery is ever allowed to fail a Series, that behavior must first become a sealed Series policy rather than a start-time switch.
