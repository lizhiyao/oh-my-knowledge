...
Independent Series analysis is assembled as `EvaluationSeriesRuntimePorts`; it never enters `EvaluationEngineRuntimeBindings`. Assembly preserves the Core `openRun` lifecycle for Series analysis and decision ports, including their per-run disposal. It does not convert them into the obsolete top-level `analyze`／`decide` callbacks.
...
## Custom executor Runtime adapter

`createCustomExecutorAdapter()` is the reference out-of-process bridge. It accepts a sealed Target and RuntimeBinding, one absolute executable path plus an explicit argument vector, and a complete classified child environment. Every environment entry is classified as public behavior identity, credential, or effect locator. Behavior identity enters Runtime facets; credential and locator values are neither persisted nor hashed. The adapter never invokes a shell, searches `PATH`, inherits `process.env`／`process.cwd()`, parses a command string, or accepts an arbitrary live directory. It selects the working directory from the exact sample-scoped Trial control: a fresh private copy of the verified workspace snapshot when requested, otherwise an empty private directory; it removes either directory when the Trial closes. These choices remove ambient host state, mutable directory locators, and quoting rules from the execution contract.

Each attempt starts one process and sends one canonical `omk.custom-executor-exchange/v1` JSON document on stdin. The document contains only Core run／trial／attempt context, the exact effective execution control, content-addressed isolation keys, and the current Trial's verified resource-lease projection. Resource entries are canonical by resource ID. The custom Runtime implementation itself is a sensitive, content-addressed resource in every executor binding; the adapter starts only its immutable Run-scoped snapshot, never the original locator. The adapter rejects a Trial control that differs from the sealed Target, requires exact binding lease coverage, and excludes the Runtime implementation plus every other sample's workspace from the child request. Gold classification and analysis-only resource kinds fail before process creation. The response is a strict, source-neutral versioned document: completed output／trace and reported usage are optional; a structured failure exposes only stable code and execution／infrastructure stage. Unknown usage remains absent, extra fields and malformed JSON fail closed, and child stderr never enters the Core error.
...
Lease acquisition snapshots all descriptors and binding requests synchronously before its first effect. It then materializes only resources requested by active bindings, copies source bytes into a private run directory, and verifies the private snapshot rather than continuing to consume the locator. Immutable snapshots are read-only. Workspace binding leases expose only a verified read-only base. Codex CLI／SDK, Claude CLI／SDK, DSH, and custom-executor adapters create a private writable copy when each Trial opens and release it when the Trial closes. An unbound Trial starts in an empty private directory. Attempts within one Trial retain that directory; different Trials and runs never share writable workspace state. No run-scoped writable overlay is allocated. The Node backend currently realizes this copy-on-write isolation contract as an eager private copy; the lease mode specifies isolation semantics, not a required filesystem mechanism. Gold is projected only through the analysis-host map.
...
Per-resource and whole-run byte／entry limits bound the acquired snapshots. Trial copies are bounded by the verified base size; this is not a quota for files subsequently created by the provider. Planned logical bytes are rejected before copying; entry limits are enforced while bounded resources are materialized. Errors carry stable codes and resource／binding identity but never include locator strings, secret bytes, or Gold content. Structurally valid inventory entries that no active binding requests are not opened, hashed, Git-probed, or copied; this preserves the no-Judge side-effect boundary.

**BREAKING-COMPARABILITY:** Trial workspace isolation corrects the former shared-run-directory behavior. Codex CLI／SDK, Claude CLI／SDK, and DSH adapter implementation versions are now `2.0.0`; custom-executor seals `trial-private-sealed-snapshot-v2` in its working-directory facet. These identities separate the corrected execution conditions from previous reports. Re-run measurements for a comparable baseline; no compatibility mode recreates the old directory sharing. Prompt bytes, scoring and statistics, Core schemas, and stored report formats are unchanged by this correction.
