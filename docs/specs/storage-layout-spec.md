...
> **Scope:** the canonical project and machine storage contract. Paths are generated by `src/shared/storage-layout.ts`; product modules must not assemble `.omk` roots themselves. This layout changes storage only and does not change report schemas, scoring, prompts, statistics, or length debiasing.
