...
Studio defaults to `$OMK_HOME/knowledge` (`~/.oh-my-knowledge/knowledge` when `OMK_HOME` is unset). An explicit page directory takes precedence over the browser’s last selection, then the default. Settings allow changing or restoring the default folder. Reading this configuration creates no directory; writes still use the shared application.
