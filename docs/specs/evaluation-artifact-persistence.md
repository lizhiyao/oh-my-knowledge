...
`omk.core-run-artifact-manifest/v2` uses qualified discriminants (`manifestKind`, `documentKind`). Its five references have fixed relative filenames, schema identity, native identity digest, and a canonical full-document digest. It also records run/report identity, run-contract digest, orthogonal Report status, Execution/Evaluation replayability, creation time, and maximum captured-content classification. Layout v2 names the authoritative report `report.json`; older manifest bundles are not read.
...
Gold comparison requires the caller to select one sealed Target, Evaluator, and numeric Metric. Optional trial and measurement coordinates refine that identity, but must match the Plan. Gold and Metric scales must match exactly. A sample resolving to multiple observations fails instead of averaging across trials, ensemble members, or replicates; a scale transformation belongs in a preregistered Analysis. The v2 projection binds a canonical Gold dataset digest and uses `null`, not non-JSON `NaN`, for undefined statistics. It records complete bootstrap-draw coverage and uses the fixed-expected-disagreement reliability bootstrap; perfect agreement has an explicit non-applicable interval. An optional, caller-selected minimum alpha produces a post-hoc assessment against the interval lower bound; there is no universal default and the assessment is not a release DecisionResult. The Krippendorff alpha, weighted kappa, and Pearson point formulas remain unchanged; the v2 interval is a `BREAKING-COMPARABILITY` change from v1's conditional-finite-draw bootstrap.
