...
The two senses of "candidate" must not be merged: on the content layer 候选知识 is an item awaiting review; on the carrier-change layer 「候选版本」 is one comparison arm inside an evaluation, i.e. the `variant` of section 2.3 above, which already states that a variant "is not a domain object itself".
