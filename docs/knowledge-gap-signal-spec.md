...
如果 spec 不明确这一点，gap rate 会被误读成"低 = 好"的单调指标。使用者会把"追求低 gap rate"当成目标，然后自然地选择不去挑战那些让 gap rate 变难看的样本——也就是**正在发生的 overfitting to the test set**。spec 必须反复强调：**gap rate 是诊断工具，不是 KPI**。
