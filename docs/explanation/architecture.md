...
`shared` is a cross-domain leaf and depends only on itself. `eval-core` is the deterministic measurement kernel on Node.js: host-neutral means it depends on no CLI, filesystem, network, environment variable, concrete executor or Studio, while the Node.js runtime itself (including `node:crypto` for canonical digests) is an accepted dependency and hashing is not injected. `eval-runtime` is the service-host runtime layer, and every module inside it is reachable from a published entry — a machine-checked invariant (`test/architecture/production-reachability.test.ts`), not a prose promise. Its canonical façade compiles ordinary `evaluate()` input into existing Core contracts, and its foundation assembles explicit ports and Core built-ins. It also owns two published surfaces that hosts call directly: the Judge／Evaluator implementations (`judges/`, `evaluators/`, `custom-evaluator.ts`) and the host-verification surface (`conformance/`: `checkExecutor`, `checkRuntime`, `run*Conformance`, plus `debugEvaluator`), which is how a host proves its own adapter conforms before shipping it. Product workflows stay in `eval-workflows`; filesystems, directories, persistence, provider runtimes, and UI remain outside Core and are assembled by hosts.
...
`eval-core/verify` owns conformance decisions: given declared contract shapes plus observed facts, it decides whether reality satisfies a plan. `contracts` keeps schema, identity and digest computation only, and must not import `verify`; the layering guard now registers only normalization and digest helpers still living in the contract layer, and that debt may only shrink.
...
The IR distinguishes `message`, `tool_call`, `tool_result`, `usage`, `lifecycle`, `agent_activity`, `observed_effect`, and `unknown` events. `observed_effect` records an effect the runtime observed (for example, which files a command changed): it is not a model-initiated call, so it never enters tool counts. User messages carry a `human`, `runtime`, `skill-context`, or `synthetic` origin, so injected instructions, environment context, and tool results cannot inflate human-turn metrics. Tool calls retain provider namespaces. Outcomes use `success`, `failure`, `cancelled`, and `unknown`; ambiguous outcomes remain unknown.
