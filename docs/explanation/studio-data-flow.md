...
- **Next.js host (`omk studio`, DSH plugin `/omk observe`, CLI eval preview)**: `next-server.ts` intercepts **every Studio page route** — `/observe` and `/observe/conversations/*`, `/observe/inbox`, the four health pages (`/observe/health`, `/observe/health/:reportId`, `/observe/health-diff`, `/observe/skill-trend/:skill`), `/measure*`, `/knowledge` and `/knowledge/skills/*`, `/knowledge/managed*`, `/agents`, plus `/_next/*` assets — and loads the page model **exactly once per request** through the `http/pages/` loaders (`loadObservePage` / `loadInboxPage` / `loadHealthPage` / `loadKnowledgePage` / `loadManagedPage` / `loadMeasurePage` / `loadAgentsPage`). Those loaders exist for this host only: they settle the 400/404/503 contract before Next starts streaming, so a route that exists but whose record is missing is always the host's plain-text 404 (#902 §三). `/agents` is the deliberate exception: a missing or unreadable `omk agents` report is a fact the page can show, so it renders "not run yet" and "report unreadable" as two distinct states instead of a 503. The shelled `not-found` document is only Next's client-side router-mismatch boundary—an address with no host route never reaches Next and gets the HTTP adapter's plain-text 404; a segment-level `notFound()` cannot hold the server contract either, because the root `loading.tsx` flushes the shell first and the status stays 200. The model is stored in an AsyncLocalStorage instance bridged through `Symbol.for` so the CLI module graph and the Next bundle resolve the same store, then renders. Server components read the snapshot via `web/catalog.tsx`; a missing store is an assembly error (`studio_context_missing` → 500), not a data-source failure (→ 503). The intercept set shrinks with the host's switches: `observationInbox: false` drops the inbox page and its APIs, `studioPages: false` drops the whole observe/knowledge page group, and dropped paths stop being intercepted so they fall through to the HTTP adapter and 404. The CLI eval preview sets `studioPages: false`: it serves only `/measure`, and the same switch makes the shell omit the primary-navigation landmark instead of linking to 404s.
- **Standalone report-server host**: `request-handler.ts` resolves the directory selection once per request and dispatches through the declarative route table (`routes/router.ts`) to JSON APIs, SSE, `/health` and plain-text status documents. It renders no page HTML — the shared HTML rendering layer was deleted once the last read-only reports moved to Next, so no page route is served here even when the Next host drops its page group; that path 404s. The Next host wraps this handler, so every non-intercepted path — including `/api/reports` (the evaluation JSON projection) and `GET /` (302 → `/observe` on hosts that mount the observe group, 404 otherwise) — is served here. `studioPages: false` also suppresses the observations directory side effect, which is why a measure-only mount never writes into the user's project.
