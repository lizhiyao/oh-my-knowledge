...
export function renderSummaryCards(variants, summary, lang) {
  ...
  const rows = variants.map((v, i) => {
    ...
    // Efficiency
    const effCell = `<td class="summary-cell"><div class="summary-value">${fmtNum(s.avgDurationMs)}<span class="summary-unit">ms</span></div></td>`;
    ...
    const rateColor = successRate === 100 ? 'var(--green)' : successRate >= 90 ? 'var(--yellow)' : 'var(--red)';
    const stabCell = `<td class="summary-cell"><div class="summary-value" style="color:${rateColor}">${successRate}%</div><div class="summary-detail">${successCount}/${total}</div></td>`;
