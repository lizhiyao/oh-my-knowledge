...
export function renderSampleTable(variants, results, report, lang) {
  ...
  const sampleRows = results.map((r) => {
    ...
    <tr style="background:rgba(15,23,42,0.4)"><td style="font-size:11px;color:#64748b" data-i18n="feedback">${t('feedback', lang)}</td>${feedbackCols}</tr>`;
  }).join('');
    ...
    return `<td colspan="3">
      <div style="display:flex;align-items:center;gap:6px;font-size:12px" id="${formId}">
        <span class="stars" style="cursor:pointer;font-size:16px" data-rating="0"
          onclick="(function(el,evt){var t=evt.target;if(t.dataset.star){var r=+t.dataset.star;el.dataset.rating=r;var s=el.children;for(var i=0;i<5;i++){s[i].style.color=i<r?'#fbbf24':'#475569'}}})(this,event)">
          <span data-star="1" style="color:#475569">&#9733;</span>
          <span data-star="2" style="color:#475569">&#9733;</span>
          <span data-star="3" style="color:#475569">&#9733;</span>
          <span data-star="4" style="color:#475569">&#9733;</span>
          <span data-star="5" style="color:#475569">&#9733;</span>
        </span>
        <input type="text" placeholder="${t('feedbackPlaceholder', lang)}" data-i18n="feedbackPlaceholder" style="font-size:12px;width:120px" />
        <button onclick="submitFeedback('${e(report.id)}','${e(r.sample_id)}','${e(v)}',this)" style="padding:3px 10px;font-size:12px" data-i18n="feedbackSubmit">${t('feedbackSubmit', lang)}</button>
      </div>
    </td>`;
  }).join('');
}
