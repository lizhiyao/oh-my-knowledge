  ...
  return report.results
    .map((r) => {
      ...
      return {
        ...
        prompt: v.outputPreview ? undefined : undefined, // prompt is not in variant result
...
export async function evolveSkill({
  ...
  const baselineReport = await evaluate(r0Path, {
    samplesPath: absSamplesPath, skillDir, model, judgeModel, executorName, concurrency, onProgress,
  });
  ...
  for (let round = 1; round <= rounds; round++) {
    ...
    const improveResult = await executor({ model: improveModel, system: IMPROVE_SYSTEM_PROMPT, prompt: improvePrompt });
...
async function evaluate(skillFilePath, { samplesPath, skillDir, model, judgeModel, executorName, concurrency, onProgress }) {
