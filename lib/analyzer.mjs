    ...
    if (baseTurns > 0 && otherTurns > 0 && baseTurns !== otherTurns) {
      ...
      if (Math.abs(turnsDiff / baseTurns) > 0.3) {
        details.push(turnsDiff > 0
          ? `${variants[i]} 平均减少 ${turnsDiff.toFixed(1)} 轮对话（↓${Math.abs(turnsPct)}%）`
          : `${variants[i]} 平均增加 ${Math.abs(turnsDiff).toFixed(1)} 轮对话（↑${Math.abs(turnsPct)}%）`);
      }
    }
    ...
    if (baseCost > 0 && otherCost > 0 && baseCost !== otherCost) {
      ...
      if (Math.abs(costDiff / baseCost) > 0.2) {
        details.push(costDiff > 0
          ? `单样本成本降低 ${Math.abs(costPct)}%（$${otherCost.toFixed(4)} vs $${baseCost.toFixed(4)}）`
          : `单样本成本增加 ${Math.abs(costPct)}%（$${otherCost.toFixed(4)} vs $${baseCost.toFixed(4)}）`);
      }
    }
