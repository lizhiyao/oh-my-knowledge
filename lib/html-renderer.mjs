...
export function renderRunList(runs) {
  ...
  const rows = runs.map((run) => {
    ...
    return `<tr>
      ...
      <td><button onclick="deleteRun('${e(run.id)}',this)" style="padding:4px 10px;border:1px solid #e2e8f0;border-radius:4px;background:#fff;cursor:pointer;font-size:12px;color:#dc2626">删除</button></td>
    </tr>`;
  }).join('');
  ...
  return layout('OMK Bench', `
    ...
    <table>
      <thead><tr><th>Run ID</th><th>Variants</th><th>Model</th><th>Samples</th><th>Score</th><th>Cost</th><th>Time</th><th></th></tr></thead>
      <tbody>${rows}</tbody>
    </table>
