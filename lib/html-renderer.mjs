...
import { e, fmtCost, fmtNum, fmtLocalTime, barChart, COLORS } from './renderer/helpers.mjs';
...
export function renderRunList(runs, lang = DEFAULT_LANG) {
  ...
  const rows = runs.map((run) => {
    ...
    return `<tr>
      ...
      <td><button onclick="deleteRun('${e(run.id)}',this)" style="padding:4px 10px;font-size:12px;color:#f87171" data-i18n="deleteBtnText">${t('deleteBtnText', lang)}</button></td>
    </tr>`;
  }).join('');
...
export function renderRunDetail(report, lang = DEFAULT_LANG) {
  ...
  return layout(`OMK Bench - ${report.id}`, `
    ...
    <div style="margin:12px 0">
      <button onclick="document.getElementById('blind-reveal').style.display=document.getElementById('blind-reveal').style.display==='none'?'block':'none'" style="padding:6px 14px;font-size:13px" data-i18n="revealBlind">${t('revealBlind', lang)}</button>
      <div id="blind-reveal" style="display:none;margin-top:8px;padding:12px;background:rgba(30,41,59,0.8);border:1px solid rgba(148,163,184,0.1);border-radius:8px;font-size:14px">
    ...
    function submitFeedback(runId, sampleId, variant, btn) {
      ...
      fetch('/api/run/' + encodeURIComponent(runId) + '/feedback', {
        ...
        if (d.ok) { btn.textContent = I18N[lang].feedbackSubmitted; btn.disabled = true; btn.style.color = '#4ade80'; }
...
export function renderEachRunDetail(report, lang = DEFAULT_LANG) {
  ...
  const overviewRows = (overview.skills || []).map((sk) => {
    ...
    const impColor = imp.startsWith('+') ? '#4ade80' : imp.startsWith('-') ? '#f87171' : '#64748b';
    return `<tr>
  ...
  const skillSections = skills.map((sk) => {
    ...
    return `
      <div id="skill-${e(sk.name)}" style="margin-top:40px;padding-top:20px;border-top:1px solid rgba(148,163,184,0.1)">
        <h2>${e(sk.name)}</h2>
        <p style="font-size:13px;color:#475569">Samples: ${sk.sampleCount} &middot; Hash: ${e(sk.skillHash || '-')}</p>
        ${cards}
        ${sampleTable}
      </div>
    `;
  ...
  return layout(`OMK Bench - ${report.id}`, `
    ...
    <p class="subtitle">${e(report.id)} &middot; ${t('model', lang)}: ${e(m.model)} &middot; ${t('judge', lang)}: ${e(m.judgeModel || 'none')} &middot; ${t('executor', lang)}: ${e(m.executor || 'claude')} &middot; ${t('cost', lang)}: ${fmtCost(m.totalCostUSD)}</p>

    <h2>Overview</h2>
    <p style="font-size:14px;color:#475569">${overview.totalSkills || 0} skills &middot; ${overview.totalSamples || 0} samples &middot; ${fmtCost(overview.totalCostUSD)}</p>
    <table>
      <thead><tr>
        <th>Skill</th>
        <th>Baseline</th>
