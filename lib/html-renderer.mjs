...
export function renderEachRunDetail(report, lang = DEFAULT_LANG) {
  ...
  const overviewRows = (overview.skills || []).map((sk) => {
    ...
    const impColor = imp.startsWith('+') ? '#16a34a' : imp.startsWith('-') ? '#dc2626' : '#6b7280';
    return `<tr>
  ...
  const skillSections = skills.map((sk) => {
    ...
    return `
      <div id="skill-${e(sk.name)}" style="margin-top:40px;padding-top:20px;border-top:2px solid #e2e8f0">
        <h2>${e(sk.name)}</h2>
        <p style="font-size:13px;color:#6b7280">Samples: ${sk.sampleCount} &middot; Hash: ${e(sk.skillHash || '-')}</p>
        ${cards}
        ${sampleTable}
      </div>
    `;
  ...
  return layout(`OMK Bench - ${report.id}`, `
    ...
    <h2>Overview</h2>
    <p style="font-size:14px;color:#6b7280">${overview.totalSkills || 0} skills &middot; ${overview.totalSamples || 0} samples &middot; ${fmtCost(overview.totalCostUSD)}</p>
    <table>
      <thead><tr>
        <th>Skill</th>
        <th>Baseline</th>
