...
import { DEFAULT_OUTPUT_DIR, generateRunId, persistReport } from '../evaluation-core.js';
...
export async function executeEachEvaluationRuns({
  ...
  for (let i = 0; i < skillEntries.length; i++) {
    ...
    const skillSummary = fullReport.summary.skill || {};
    const artifactHash = fullReport.meta.artifactHashes?.skill || '';
