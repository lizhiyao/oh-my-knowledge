...
export function createReportServer({ port, reportsDir = DEFAULT_REPORTS_DIR, store } = {}) {
  ...
  async function handleFeedback(req, res, runId) {
    try {
      ...
      // Atomic read-modify-write with mutex lock in store
      const updated = await reportStore.update(runId, (report) => {
        const result = (report.results || []).find((r) => r.sample_id === sample_id);
        if (!result) throw new Error('SAMPLE_NOT_FOUND');
      ...
      if (err.message === 'SAMPLE_NOT_FOUND') {
        res.writeHead(404, { 'Content-Type': 'application/json' });
