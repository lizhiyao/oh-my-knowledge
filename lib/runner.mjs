...
export function loadSkills(skillDir, variants, executorName = 'claude') {
  const skills = {};
...
async function executeTasks({ tasks, executor, judgeExecutor, model, judgeModel, noJudge, samplesPath, concurrency, onProgress, executorName }) {
  const results = {};
  let started = 0;
  let completed = 0;
  let totalCostUSD = 0;
  ...
  async function executeTask(task) {
    ...
    const execResult = await executor({
      model,
      system: executorName === 'script' ? undefined : task.skillContent,
      skillPath: executorName === 'script' ? task.skillContent : undefined,
      prompt: task.prompt,
    });
    ...
    meta: {
      ...
      skillHashes: Object.fromEntries(
        Object.entries(skills).map(([name, content]) => {
          if (existsSync(join(String(content), 'SKILL.md'))) {
...
export async function runEvaluation({
  ...
  const { results, totalCostUSD } = await executeTasks({
    tasks, executor, judgeExecutor, model, judgeModel, noJudge, samplesPath, concurrency, onProgress, executorName,
  });
