...
async function executeTasks({ tasks, executor, judgeExecutor, model, judgeModel, noJudge, samplesPath, concurrency, timeoutMs, onProgress }) {
  const results = {};
  let started = 0;
  let completed = 0;
  let totalCostUSD = 0;
  ...
  async function executeTask(task) {
    ...
    const execResult = await executor({ model, system: task.skillContent, prompt: task.prompt, cwd: task.cwd, timeoutMs });
...
export async function runEvaluation({
  ...
  const { results, totalCostUSD } = await executeTasks({
    tasks, executor, judgeExecutor, model, judgeModel, noJudge, samplesPath, concurrency, timeoutMs, onProgress,
  });
