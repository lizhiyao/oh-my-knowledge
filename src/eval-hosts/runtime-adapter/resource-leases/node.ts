  ...
  const walk = async (source: string, destination: string, segments: string[]): Promise<void> => {
    ...
    for await (const entry of directory) {
      ...
      if (stat.isDirectory()) {
        ...
        await copyRegularFile(resourceId, sourcePath, destinationPath, state, limits, signal);
      }
    }
  };
...
async function hashFile(path: string, hash: Hash, signal?: AbortSignal): Promise<number> {
...
async function digestFile(path: string, signal?: AbortSignal): Promise<{ digest: `sha256:${string}`; size: number }> {
  const hash = createHash('sha256');
  const size = await hashFile(path, hash, signal);
  ...
  const walk = async (directory: string, segments: string[]): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) {
        ...
        size += await hashFile(childPath, hash, signal);
...
class NodePinnedGitVerifier implements OmkPinnedGitVerifier {
  async verifyPinnedCommit(input: {
    ...
    const options = { encoding: 'utf8' as const, maxBuffer: 4 * 1024 * 1024, signal: input.signal };
...
async function materializeBase(
  ...
  if (expectsDirectory) {
    ...
    actual = await digestTree(destination, false, signal);
    ...
    actual = await digestFile(destination, signal);
    snapshotKind = 'file';
