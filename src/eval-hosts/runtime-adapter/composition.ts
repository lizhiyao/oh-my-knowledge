...
import { createJudgeAggregationSchemaValidators } from '../../eval-workflows/measurement/analysis/judge-aggregation.js';
import { createAssertionLayerParameterSchemaValidators } from '../../eval-workflows/measurement/analysis/assertion-layer-parameters.js';
import { createAssertionLayerTableSchemaValidators } from '../../eval-workflows/measurement/analysis/assertion-layer.js';
import { createDimensionParameterSchemaValidators } from '../../eval-workflows/measurement/analysis/dimension-parameters.js';
import { createDimensionTableSchemaValidators } from '../../eval-workflows/measurement/analysis/dimension-table.js';
import { createCompositeParameterSchemaValidators } from '../../eval-workflows/measurement/analysis/composite-parameters.js';
import { createCompositeTableSchemaValidators } from '../../eval-workflows/measurement/analysis/composite-table.js';
import {
  createBootstrapFamilyParameterSchemaValidators,
} from '../../eval-workflows/measurement/analysis/bootstrap-family-parameters.js';
import {
  createBootstrapFamilyTableSchemaValidators,
} from '../../eval-workflows/measurement/analysis/bootstrap-family-table.js';
import {
  createBootstrapFamilyTableV2SchemaValidators,
} from '../../eval-workflows/measurement/analysis/bootstrap-family-table-v2.js';
import { createAgreementParameterSchemaValidators } from '../../eval-workflows/measurement/analysis/agreement-parameters.js';
import { createAgreementTableSchemaValidators } from '../../eval-workflows/measurement/analysis/agreement-table.js';
import {
  createReleaseDecisionParameterSchemaValidators,
} from '../../eval-workflows/measurement/analysis/release-decision-parameters.js';
...
import { createOmkSeriesVarianceSchemaValidators } from '../../eval-workflows/measurement/analysis/series-variance.js';
import {
...
async function eventWriterForRun(input: {
  ...
  try {
    return capturePort(
      await input.factory(Object.freeze({ runId: input.runId, plan: input.plan, signal: input.signal })),
      ['write'],
...
export async function createOmkEvaluationRuntime(
  ...
  const execution = createEvaluationExecution({
    ...
    async acquireRun({ runId, plan, signal }) {
      let leases: OmkRunResourceLeases | undefined;
      let registered = false;
      const close = async (): Promise<void> => {
        const failures: unknown[] = [];
        if (registered) {
          try { assembly.evaluation.resourceLeaseRegistry.unregister(runId); }
          catch (cause) { failures.push(cause); }
          registered = false;
        }
        try { await leases?.dispose(); } catch (cause) { failures.push(cause); }
        if (failures.length > 0) throw new AggregateError(failures, '运行资源清理失败。');
      };
      try {
        try {
          leases = await resources.materialize({
            ...
            ...(resources.limits === undefined ? {} : { limits: resources.limits }),
            ...(resources.pinnedGitVerifier === undefined ? {} : {
              pinnedGitVerifier: resources.pinnedGitVerifier,
            }),
          });
        } catch (cause) {
          if (cause instanceof OmkResourceLeaseError || signal.aborted) throw cause;
          return fail({
            code: 'OMK_EVALUATION_RUNTIME_RESOURCE_LEASE_FAILED', runId,
            message: 'Run resource lease acquisition 失败。',
          });
        }
        leases = captureMaterializedLeases(leases, runId, analysisOnly, compiled.hostResources);
        // Return the lease even after cancellation; Runtime owns its single cleanup attempt.
        const eventWriter = signal.aborted ? undefined : await eventWriterForRun({
          factory: eventWriterFactory, writerMode: compiled.policy.eventDelivery.writerMode,
          runId, plan, signal,
        });
        return Object.freeze({
        ...
        try { await close(); } catch (cleanupCause) {
          throw new AggregateError([cause, cleanupCause], '资源获取失败后的清理失败。');
        }
        throw cause;
      }
    },
...
/** Outer-host adapter from product compilation to Runtime-owned execution capabilities. */
export function createOmkRuntimeProvider(
  input: Readonly<CreateOmkEvaluationRuntimeInput>,
  ...
  return Object.freeze<EvaluationRuntimeProvider>({
        ...
        compiled: {
          ...
          runOptions: {
            metadata: {
              ...
              ...(request.summaries === undefined ? {} : { summaries: request.summaries }),
            },
          },
          canonicalDigests: {
            definition: digestCanonicalJson(request.definition),
            policy: digestCanonicalJson(request.policy),
          },
        },
      });
    ...
    async prepareSeries(definition) {
      const runtime = await createOmkEvaluationRuntime({ compiled, factories, support, resources });
      if (runtime.series === undefined) throw new TypeError('宿主未提供 Series 运行能力。');
      return prepareRuntimeSeries({
        definition,
        runtimes: runtime.series.runtimes,
        ports: {
          ...runtime.series.ports,
