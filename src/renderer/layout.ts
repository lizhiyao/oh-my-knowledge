...
export const I18N: Record<Lang, Record<string, string>> = {
  zh: {
    ...
    judgeStddev: '判官波动', judgeStddevDesc: '同一份输出让 judge 判 N 次 (--judge-repeat) 得到 N 个分数的标准差。值低 = judge 对自己很坚定；值高 = 这个分本身就是噪声',
    judgeFailures: '判官失败', judgeFailuresDesc: 'N 次 judge 中返回 score=0（解析失败 / 调用错误）的次数。stddev=0 + failureCount>0 不是"完美一致"，是"大部分炸了"',
    judgeReasoning: 'CoT 推理', judgeReasoningExpand: '展开',
    ensembleHeader: '多 judge 评分对比', ensembleDesc: '不同 judge 模型对同一份输出的独立评分。用于反驳"同模态偏差"',
    agreementHeader: '跨样本判官一致性', agreementDesc: '在所有样本上算的 inter-judge agreement',
    pearsonLabel: 'Pearson', pearsonDesc: '皮尔逊相关系数：1=完全同向排序，0=无关，-1=完全反向',
    madLabel: 'MAD', madDesc: '平均绝对差。1-5 制下 < 0.5 紧密一致, > 1.5 大分歧',
    judgeModelsLabel: '评委模型',
    judgeRepeatLabel: '每条 judge 次数',
    judgePromptHashLabel: '判官 prompt 指纹', judgePromptHashDesc: '判官 prompt 模板的 SHA256 前 12 位。两份报告 hash 相同才能严格比 score',
    sampleHashCount: '样本指纹覆盖',
  ...
  en: {
    ...
    sampleHashCount: 'Sample fingerprints',
