...
export default class ObserveInbox extends Command {
  ...
  static flags = {
    ...
    refresh: Flags.boolean({
      description: bilingual({
        zh: '重新抽取软标准候选，不复用已有抽取结果',
        en: 'Refresh soft standard extraction instead of reusing existing records',
      }),
      default: false,
    }),
    model: Flags.string({
      description: bilingual({
        zh: '抽取软标准使用的模型，默认 sonnet',
        en: 'Model for soft standard extraction, default sonnet',
      }),
    }),
    executor: Flags.string({
      description: bilingual({
        zh: '抽取软标准使用的执行器',
        en: 'Executor for soft standard extraction',
      }),
    }),
    json: Flags.boolean({
