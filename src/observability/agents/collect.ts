 ...
 * 单文件上限仍是内存边界：峰值约等于 1.1 倍最大可采文件（改造前约 2.7～2.9 倍；本机最大的那份
 * 1.4 GB 日志真机复跑三次是 1 487／1 569／1 569 MiB，按 1.16 倍留余量）。抬高它既抬高可采的最大
 * 文件，也按比例抬高内存天花板；要把峰值稳在某个界限内，就按这个系数反推上限。
