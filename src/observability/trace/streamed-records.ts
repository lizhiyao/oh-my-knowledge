 ...
 * 下来的事件——同一份文件只跑一遍「取完就丢」的遍历，maximum resident 就到 1 512 MiB（结束时堆仅 139 MiB），
 * 再跑 12 遍也只多 8%。量级就是「每条记录先解成一个 JS 字符串」这一遍分配；要再往下压得换成
 * 不逐条分配字符串的流式分词读取，而不是继续改事件的生产方式，见 #974 的复核记录。
 *
