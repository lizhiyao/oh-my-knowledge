 ...
 * （1 449 MiB，此时 heapUsed 36 MiB）。按「每行做什么」拆开量（同一份 1 357 MiB 日志、83 182 行、
 * 读缓冲复用）：纯字节扫 55 MiB、只把行解码成 JS 字符串 133 MiB、加上 `JSON.parse` 才 1 400 MiB——
 * 撑住驻留的是逐条建出的对象图（比只解码那一档多 1 267 MiB，≈文件大小
 * 的 0.9 倍），且这些页 V8 不还给系统（old space 限到 64 MiB 后 heapUsed 只剩 16 MiB，maximum
 * resident 仍是 1 155 MiB，另两次独立跑 1 192／1 195 MiB，所以「子进程＋小堆」救不了；把完整 GC
 * 提到每 200 条一次、再叠加堆上限与半空间也只降到 920 MiB（heapUsed 5 MiB）——调回收节奏改的是
 * 系数，不是斜率。要再往下压得让读取层不逐条建对象图：在字节缓冲上按需取出真正进 IR 的字段。
 * 见 #974 的复核记录。
 *
