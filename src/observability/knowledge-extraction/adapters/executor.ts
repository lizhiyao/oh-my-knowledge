...
export function configuredExtractionModel(executor: string, model: string, implementation?: ExecutorFn): ExtractionModel {
  if (!['codex', 'claude', 'claude-sdk', 'openai-api', 'anthropic-api'].includes(executor)) {
  ...
  return {
    ...
    async generate(system, input, signal) {
      ...
      const cwd = mkdtempSync(join(tmpdir(), 'omk-knowledge-executor-'));
      try {
        const execute = implementation ?? createExecutor(executor);
        const result = await execute({ model, system, prompt: input, cwd,
          lean: true, textOnly: true, ...(executor === 'codex' ? { allowedSkills: [] } : {}),
