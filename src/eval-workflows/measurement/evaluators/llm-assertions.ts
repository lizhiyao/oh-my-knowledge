...
import type { SameProcessEvaluatorImplementation } from '../../../eval-runtime/adapters/same-process.js';
import {
...
export function parseLlmAssertionConfig(value: unknown): LlmAssertionConfig {
  if (!isRecord(value)
...
export function createLlmAssertionEvaluatorImplementation(
  invocation: OmkLlmJudgeInvocationPort,
): SameProcessEvaluatorImplementation<undefined, RecordState, unknown> {
  const capturedInvocation = captureLlmJudgeInvocationPort(invocation);
  const implementation: SameProcessEvaluatorImplementation<undefined, RecordState, unknown> = {
    openRun: () => undefined,
    openRecord({ record }): RecordState {
      ...
      const config = parseLlmAssertionConfig(record.evaluatorConfig);
