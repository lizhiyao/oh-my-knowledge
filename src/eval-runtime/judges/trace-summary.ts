...
function toolCallStatus(toolCall: ToolCallInfo): 'success' | 'failure' | 'cancelled' | 'unknown' {
  return toolCall.status ?? (toolCall.success ? 'success' : 'failure');
}
...
export function buildJudgeTraceSummary(
  ...
  if (toolCalls && toolCalls.length > 0) {
    ...
    const successCount = toolCalls.filter((call) => toolCallStatus(call) === 'success').length;
    const failureCount = toolCalls.filter((call) => toolCallStatus(call) === 'failure').length;
    const cancelledCount = toolCalls.filter((call) => toolCallStatus(call) === 'cancelled').length;
    const unknownCount = toolCalls.filter((call) => toolCallStatus(call) === 'unknown').length;
