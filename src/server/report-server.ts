...
function renderObservationInboxPage(observationsDir: string, lang: Lang = DEFAULT_LANG): string {
  ...
  const confidenceHeaderHelp = '判断把握：OMK 对“这条 observation 是否需要处理/是否高风险/需关注”的规则判断有多确定。归属把握：OMK 把这条 observation 归到当前 skill 名下有多确定，例如明确调用 skill 通常更高。';
  ...
  const skillRollupRows = skillRollups.map((row) => {
    ...
    return `<tr data-observe-rollup-row data-skill-anchor="${e(skillAnchor(row.skillName))}" style="${hasDetail ? 'cursor:pointer' : ''}">
      ...
      <td style="padding:9px 10px;color:var(--text-secondary);font-size:12px;line-height:1.55">
        ...
        <span title="observation 中，agent 的回答出现“不确定、可能、需要确认”等表达的次数">回答不确定 ${row.metricCounts.uncertainty}</span> ·
        <span title="observation 中，agent 明确说“未知/知识缺口/没覆盖”等内容的次数">明确说缺口 ${row.metricCounts.explicitMarker}</span> ·
        <span title="observation 中，Bash 命令看起来是在试目录、试路径或探测环境的次数">Bash试探 ${row.metricCounts.bashProbe}</span><br>
        <span title="observation 中，访问的文件或路径不存在的次数">路径不存在 ${row.metricCounts.notFound}</span> ·
        <span title="observation 中，文件太长、token 上限或超时这类工具限制次数">工具限制 ${row.metricCounts.toolLimit}</span> ·
        <span title="observation 中，工具调用失败但不属于路径不存在或工具限制的次数">工具失败 ${row.metricCounts.toolFailure}</span>
      </td>
      ...
      <td style="padding:9px 10px;color:var(--text-muted);font-size:12px" title="${row.lastProblemSeen ? '最近一次产生 observation 的时间' : '当前没有发现问题或旧 report 没有 observation 时间'}">${row.lastProblemSeen ? e(row.lastProblemSeen.slice(0, 19).replace('T', ' ')) : '—'}</td>
      <td style="padding:9px 10px;color:var(--text-muted);font-size:12px" title="${row.lastUsed ? '最近一次在 trace 中识别到 skill 调用的时间' : '当前 observation report 中没有这个 skill 的调用时间信息；旧 report 需要重新 ingest 才会补齐'}">${row.lastUsed ? e(row.lastUsed.slice(0, 19).replace('T', ' ')) : '—'}</td>
  ...
  const funnelRows = [
    ...
    },
    {
      ...
      status: '当前不能统计',
      count: '—',
      desc: '当前不统计。还没有把 SKILL.md 的版本、改动时间和 observation 趋势关联起来，所以不能判断 v2 是否比 v1 更好。',
    },
  ];
    ...
    <main style="max-width:1120px;margin:0 auto;padding:24px">
      ...
      <section style="margin-top:16px;border:1px solid var(--border);border-radius:8px;background:var(--bg-surface);overflow:hidden">
        <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;padding:13px 14px;border-bottom:1px solid var(--border)">
          <div>
            ...
            <div style="color:var(--text-muted);font-size:12px;margin-top:3px">一行一个 skill。子项指标同时汇总 trace 工具调用和 observation 信号：工具调用看运行行为，observation 看发现的问题类型。</div>
          </div>
        ...
        <div style="width:100%;max-height:70vh;overflow:auto">
          <table style="border-collapse:collapse;width:100%;min-width:1340px;font-size:13px;table-layout:fixed;border:0;border-radius:0;background:transparent">
            ...
            <thead><tr>
              ...
              <th style="text-align:right;padding:9px 10px;border-bottom:1px solid var(--border)">Observation</th>
      ...
      <section style="margin-top:16px;display:grid;grid-template-columns:minmax(0,1.25fr) minmax(280px,.75fr);gap:12px;align-items:start">
        <section id="observe-action-panel" style="border:1px solid var(--border);border-radius:8px;background:var(--bg-surface);overflow:hidden;display:flex;flex-direction:column">
          ...
          </div>` : `<div style="padding:14px;color:var(--text-muted);font-size:13px">当前没有需要 review 的 observation。</div>`}
        </section>
