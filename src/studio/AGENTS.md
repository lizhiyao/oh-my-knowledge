...
- 悬停反馈按表面分两档：正文与弹层里的链接走品牌深色，由低优先级默认值给出（`.studio-content :where(a:hover)`、`.ant-modal-root`、`.ant-drawer`），自带悬停设计的组件覆盖它；一级导航、分区导航与侧栏走墨色，悬停 `#293348`，当前页 `#192236` 并以品牌色下边框标识。当前页不参与悬停加深，否则最深的那一档反而变浅。任何可点表面都不得既无颜色变化也无背景变化——导航曾长期只有 `aria-current` 与焦点环，悬停时与不可点文字没有区别。
