...
- 悬停反馈按表面分两档：正文与弹层里的链接走品牌深色，由低优先级默认值给出（`.studio-content :where(a:hover)`、`.ant-modal-root`、`.ant-drawer`），自带悬停设计的组件覆盖它；一级导航、分区导航与侧栏走墨色，悬停 `#293348`，当前页 `#192236`。一级导航收在外壳侧栏（#1055），当前页以背景块标识，不使用页头时代的品牌色下边框。当前页不参与悬停加深，否则最深的那一档反而变浅。任何可点表面都不得既无颜色变化也无背景变化——导航曾长期只有 `aria-current` 与焦点环，悬停时与不可点文字没有区别。
- 外壳左侧栏是品牌、一级导航、工作区列表与设置入口的唯一载体（#1055）：工作区把列表内容经 `StudioShell` 的 `sidebar` 槽位交给外壳渲染，页面与工作区组件不自画 `StudioUtilities`，也不在外壳之外另起导航侧栏。预览宿主（只挂 `/measure`、无兄弟路由）保留无导航的紧凑页头，不侧栏化。
