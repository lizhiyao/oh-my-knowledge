...
export function StudioShell({ lang, children, active = 'measure' }: { lang: Language; children: ReactNode; active?: 'observe' | 'measure' | 'knowledge' }) {
  const suffix = lang === 'en' ? '?lang=en' : '';
  return <ConfigProvider locale={lang === 'zh' ? zhCN : enUS}>
    <div className="studio-app"><header className="studio-header"><a className="studio-brand" href={`/${suffix}`} aria-label="OMK Studio"><span className="studio-mark">omk</span><span>Studio</span></a>
