...
const COPY = {
  zh: {
    ...
    unreadable: '不可读',
    truncated: '已截断',
    detectedSummary: (known: number, installed: number, files: number) => `登记表 ${known} 个 · 已安装 ${installed} 个 · 来源会话日志 ${files} 份`,
    collectedSummary: (collected: number, events: number) => `采集 ${collected} 份来源会话 · 事件 ${events} 条`,
    sessionTitle: '来源会话',
    events: '事件',
    unknownEvents: '未识别事件',
    ...
    nextList: [
      ...
      { command: 'omk agents collect', text: '增量采集来源会话日志并映射成统一 Trace IR 产物。' },
      { command: 'omk agents extract --session <runId>', text: '对选定来源会话提炼实体与候选知识。' },
    ],
