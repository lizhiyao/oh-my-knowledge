...
import { Alert, Collapse, Descriptions, Empty, Input, Table, Tag, Typography } from 'antd';
...
export function RunDetail({detail,lang}: {detail:CoreStudioRunDetail;lang:Language}) {
  ...
  return <>
    ...
    <section className="measure-section"><h2>{zh?'评测范围':'Evaluation scope'}</h2><Descriptions bordered column={{xs:1,sm:2,lg:3}} items={[
    ...
    <section className="measure-section"><h2>{zh?'分析结果':'Analysis results'}</h2><Table className="measure-table" rowKey="resultId" dataSource={[...detail.stages.analysis.records]} scroll={{x:700}} columns={[
    ...
    <section className="measure-section"><Collapse items={[
