...
export function createKnowledgeCandidateRoutes(liveStreams: LiveStreamRegistry) {
  return createStudioRouter([{
    ...
    async handler({ request, response }) {
      ...
      try {
        const result = await executeKnowledgeCandidateAction(input, controller.signal);
