...
const INDEX_PATH = '/measure';
const DETAIL_PREFIX = '/measure/';

/** 运行 id 是单段稳定身份；畸形或越段一律按缺页处理，不拿去查数据源。 */
function runIdOf(path: string): string | undefined {
  if (!path.startsWith(DETAIL_PREFIX)) return undefined;
  const encoded = path.slice(DETAIL_PREFIX.length);
...
export async function loadMeasurePage(catalog: CoreStudioCatalog, path: string): Promise<MeasurePage | undefined> {
  if (path === INDEX_PATH) return { pageKind: 'index', runs: await catalog.list() };
