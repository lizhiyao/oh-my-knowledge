...
function setupBrokenSkillDir(): string {
  ...
  writeFileSync(join(tmp, 'eval-samples.json'), JSON.stringify({
    ...
    samples: [
{ sampleId: 's1', input: { inputKind: 'text', text: 'test prompt' }, evaluationContext: { assertions: [{ type: 'contains', value: 'test' }] } }
],
  }));
  return tmp;
}
...
function setupBatchMixedSkillDir(): string {
  ...
  writeFileSync(join(skillDir, 'broken', '.omk', 'eval-samples.json'), JSON.stringify({
    schemaVersion: 'omk.eval-sample-set/v3',
    samples: [
{ sampleId: 'b1', input: { inputKind: 'text', text: 'test prompt for broken' }, evaluationContext: { assertions: [{ type: 'contains', value: 'test' }] } }
],
  }));
  ...
  writeFileSync(join(skillDir, 'healthy', '.omk', 'eval-samples.json'), JSON.stringify({
    schemaVersion: 'omk.eval-sample-set/v3',
    samples: [
{ sampleId: 'h1', input: { inputKind: 'text', text: 'test prompt for healthy' }, evaluationContext: { assertions: [{ type: 'contains', value: 'test' }] } }
],
  }));
  return tmp;
}
  ...
  it('doctor preflight only checks variants used in this run, not unrelated drafts in the same skill-dir', async () => {
    ...
    try {
      ...
      writeFileSync(join(tmp, 'eval-samples.json'), JSON.stringify({
        schemaVersion: 'omk.eval-sample-set/v3',
        samples: [
{ sampleId: 's1', input: { inputKind: 'text', text: 'review' }, evaluationContext: { assertions: [{ type: 'contains', value: 'review' }] } }
],
      }));
