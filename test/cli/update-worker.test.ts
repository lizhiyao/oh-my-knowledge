...
it('compiled worker queries both publishing channels and preserves notification state', async () => {
  ...
  try {
    ...
    for (const channel of ['next', 'latest']) {
      ...
      const output = await runCli([cache, `http://127.0.0.1:${address.port}`, 'oh-my-knowledge', channel], {
        entry: worker, cwd: root, timeout: 7000, env: { ...process.env, HOME: root, OMK_HOME: root },
      });
