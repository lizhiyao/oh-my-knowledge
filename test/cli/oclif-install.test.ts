...
async function runInstallCommand(
  ...
  assert.deepEqual(args.slice(0, 2), [CLI, 'install']);
  return runCommand(InstallCommand, args.slice(2), options);
}
...
describe('oclif install', () => {
  ...
  it('installs omk-agent-skill into a custom skill root', async () => {
    ...
    try {
      ...
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--dest', dest]);
  ...
  it('prints runtime messages in English', async () => {
    ...
    try {
      const dest = join(dir, 'skills-root');
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--dest', dest, '--lang', 'en']);
  ...
  it('auto installs into detected local supported targets', async () => {
    ...
    try {
      ...
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill'], {
  ...
  it('auto detects AGENTS root directories, not same-named files', async () => {
    ...
    try {
      ...
      try {
        await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill'], {
          env: cliEnv({ HOME: home }),
        });
  ...
  it('supports explicit multiple targets', async () => {
    ...
    try {
      const home = join(dir, 'home');
      await mkdir(home, { recursive: true });
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--to', 'codex,claude'], {
        env: cliEnv({ HOME: home }),
      });
      assert.equal((stdout.match(/已安装 omk Agent Skill/g) ?? []).length, 2, `stdout should list two installs:\n${stdout}`);
      assert.ok(existsSync(join(home, '.agents', 'skills', 'omk', 'SKILL.md')), 'Codex/AGENTS target not installed');
  ...
  it('supports --to all without detected targets', async () => {
    ...
    try {
      const home = join(dir, 'home');
      await mkdir(home, { recursive: true });
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--to', 'all'], {
        env: cliEnv({ HOME: home }),
      });
      assert.equal((stdout.match(/已安装 omk Agent Skill/g) ?? []).length, 2, `stdout should list two installs:\n${stdout}`);
      assert.ok(existsSync(join(home, '.agents', 'skills', 'omk', 'SKILL.md')), 'Codex/AGENTS target not installed');
  ...
  it('supports explicit single targets', async () => {
    for (const target of ['codex', 'claude'] as const) {
      ...
      try {
        const home = join(dir, 'home');
        await mkdir(home, { recursive: true });
        const { stdout } = await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--to', target], {
          env: cliEnv({ HOME: home }),
        });
  ...
  it('rejects unknown install targets', async () => {
    try {
      await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--to', 'gemini']);
      assert.fail('expected non-zero exit');
    } catch (err) {
      const e = err as ExecError;
      assert.notEqual(e.code, 0);
  ...
  it('rejects mixed auto/all target combinations', async () => {
    for (const to of ['auto,codex', 'all,claude']) {
      try {
        await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--to', to]);
  ...
  it('refuses to overwrite existing install without --force', async () => {
    ...
    try {
      const dest = join(dir, 'skills-root');
      await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--dest', dest]);
      try {
        await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--dest', dest]);
        assert.fail('expected non-zero exit');
      } catch (err) {
  ...
  it('preflights existing targets before multi-target install', async () => {
    ...
    try {
      ...
      try {
        await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--to', 'codex,claude'], {
          env: cliEnv({ HOME: home }),
        });
        assert.fail('expected non-zero exit');
      } catch (err) {
  ...
  it('--force overwrites existing install', async () => {
    ...
    try {
      ...
      await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--dest', dest, '--force']);
  ...
  it('--dry-run prints target and does not write files', async () => {
    ...
    try {
      const dest = join(dir, 'skills-root');
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'omk-agent-skill', '--dest', dest, '--dry-run']);
  ...
  it('unknown bare token exits non-zero and points at supported inputs', async () => {
    try {
      await runInstallCommand('node', [CLI, 'install', 'other-skill']);
      assert.fail('expected non-zero exit');
    } catch (err) {
      const e = err as ExecError;
      assert.notEqual(e.code, 0);
  ...
  it('directory-skill:分发整目录 + 登记受管记录(--kind 可省自动推导)', async () => {
    ...
    try {
      ...
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'skills/review', '--dest', dest], {
        cwd: dir,
        env: cliEnv(),
      });
  ...
  it('file-skill:目标是 .md 文件', async () => {
    ...
    try {
      ...
      const dest = join(dir, 'dist-skills');
      await runInstallCommand('node', [CLI, 'install', 'notes.md', '--kind', 'skill', '--dest', dest], {
        cwd: dir,
        env: cliEnv(),
      });
  ...
  it('--dry-run 既不分发也不登记', async () => {
    ...
    try {
      await makeDirSkill(dir, 'review');
      const dest = join(dir, 'dist-skills');
      await runInstallCommand('node', [CLI, 'install', 'skills/review', '--dest', dest, '--dry-run'], {
        cwd: dir,
        env: cliEnv(),
      });
  ...
  it('--force 重装幂等:仍是一条记录,分发不重复', async () => {
    ...
    try {
      await makeDirSkill(dir, 'review');
      const dest = join(dir, 'dist-skills');
      await runInstallCommand('node', [CLI, 'install', 'skills/review', '--dest', dest], { cwd: dir, env: cliEnv() });
      await runInstallCommand('node', [CLI, 'install', 'skills/review', '--dest', dest, '--force'], { cwd: dir, env: cliEnv() });
      const record = await readSoleManagedRecord(dir);
      const distribution = record.distribution as Array<Record<string, unknown>>;
  ...
  it('就地接管:源即目标时不删源、登记成功(P1 数据丢失防护)', async () => {
    for (const force of [true, false]) {
      ...
      try {
        ...
        await runInstallCommand('node', [CLI, ...args], { cwd: dir, env: cliEnv() });
        // 源必须还在(绝不能被自删)
  ...
  it('名为 evolve 的 directory-skill 正常安装,不被根目录过滤误伤(P2 边界)', async () => {
    ...
    try {
      ...
      const dest = join(dir, 'dist-skills');
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'skills/evolve', '--dest', dest], { cwd: dir, env: cliEnv() });
  ...
  it('源嵌套在目标内:拒绝执行、绝不删源(P1 数据丢失防护)', async () => {
    ...
    try {
      ...
      try {
        await runInstallCommand('node', [CLI, 'install', 'skills/review/review', '--dest', 'skills', '--force'], { cwd: dir, env: cliEnv() });
        assert.fail('expected non-zero exit');
      } catch (err) {
        const e = err as ExecError;
        assert.notEqual(e.code, 0);
  ...
  it('裸名匹配到 cwd 普通文件时不被当 skill 安装,落回 unknown_input', async () => {
    ...
    try {
      ...
      try {
        await runInstallCommand('node', [CLI, 'install', 'omk-agnt-skill'], { cwd: dir, env: cliEnv() });
        assert.fail('expected non-zero exit');
      } catch (err) {
        const e = err as ExecError;
        assert.notEqual(e.code, 0);
  ...
  it('显式 --kind prompt 报友好错误且不登记', async () => {
    ...
    try {
      ...
      const dest = join(dir, 'dist-skills');
      try {
        await runInstallCommand('node', [CLI, 'install', 'skills/review', '--kind', 'prompt', '--dest', dest], {
          cwd: dir,
          env: cliEnv(),
        });
        assert.fail('expected non-zero exit');
  ...
  it('git 源:从当前仓库 ref 安装,分发整树 + 登记 sourceKind:git', async () => {
    ...
    try {
      ...
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'git:HEAD:skills/review', '--dest', dest], { cwd: repo, env: cliEnv() });
      assert.ok(stdout.includes('已安装 skill review'), `stdout missing copy msg:\n${stdout}`);
  ...
  it('git 源 --force 重装幂等:仍一条记录、分发不重复', async () => {
    ...
    try {
      const dest = join(repo, 'dist-skills');
      await runInstallCommand('node', [CLI, 'install', 'git:HEAD:skills/review', '--dest', dest], { cwd: repo, env: cliEnv() });
      await runInstallCommand('node', [CLI, 'install', 'git:HEAD:skills/review', '--dest', dest, '--force'], { cwd: repo, env: cliEnv() });
      const record = await readSoleManagedRecord(repo);
  ...
  it('git 源 --dry-run 不分发不登记,文案源中性', async () => {
    ...
    try {
      const dest = join(repo, 'dist-skills');
      const { stdout } = await runInstallCommand('node', [CLI, 'install', 'git:HEAD:skills/review', '--dest', dest, '--dry-run'], { cwd: repo, env: cliEnv() });
      assert.ok(!existsSync(join(dest, 'review')), 'dry-run must not distribute');
  ...
  it('git 源裸 spec 歧义:文件优先(command 集成记录 isDirectorySkill:false,防 evidence 静默剥离)', async () => {
    ...
    try {
      ...
      const dest = join(repo, 'dist-skills');
      await runInstallCommand('node', [CLI, 'install', 'git:HEAD:skills/dual', '--dest', dest], { cwd: repo, env: cliEnv() });
      const record = await readSoleManagedRecord(repo);
      const source = record.source as Record<string, unknown>;
  ...
  it('git 源在非 git 仓库内友好报错', async () => {
    ...
    try {
      try {
        await runInstallCommand('node', [CLI, 'install', 'git:HEAD:review'], { cwd: dir, env: cliEnv() });
        assert.fail('expected non-zero exit');
      } catch (err) {
        const e = err as ExecError;
        assert.notEqual(e.code, 0);
