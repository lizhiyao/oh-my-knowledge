...
describe('judge prompt versioning', () => {
  it('default builds the v6-multi-cot-toolargs-fmt-len prompt', () => {
    const text = buildJudgePrompt([{ schemaVersion: 'omk.rubric-judge-context/v2', metricId: 'score', criterionId: 'criterion', prompt: 'p', rubric: 'r' }], 'o', null);
    assert.match(text, /v6-multi-cot-toolargs-fmt-len/);
    assert.match(text, /长度不是质量信号/);
    assert.match(text, /排版与语气不是质量信号/);
  });

  it('lengthDebias=false builds the v6-multi-cot-toolargs-fmt prompt without the length-debias section', () => {
    const text = buildJudgePrompt([{ schemaVersion: 'omk.rubric-judge-context/v2', metricId: 'score', criterionId: 'criterion', prompt: 'p', rubric: 'r' }], 'o', null, false);
    // 全角 `）` 紧跟 fmt,确保是 off 版本而非 -len 变体(后者是 fmt 的超串)。
    assert.match(text, /template v6-multi-cot-toolargs-fmt）/);
  ...
  it('trace summary section is independent of debias section (both can be present)', () => {
    const text = buildJudgePrompt([{ schemaVersion: 'omk.rubric-judge-context/v2', metricId: 'score', criterionId: 'criterion', prompt: 'p', rubric: 'r' }], 'o', 'trace info here', true);
