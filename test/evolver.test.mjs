...
import { extractWeakSamples, buildImprovementPrompt, evolveSkill } from '../lib/evolver.mjs';
...
describe('extractWeakSamples', () => {
  ...
  it('returns samples sorted by score ascending', () => {
    const weak = extractWeakSamples(mockReport, 'skill');
  ...
  it('respects count limit', () => {
    const weak = extractWeakSamples(mockReport, 'skill', 2);
  ...
  it('includes dimension scores', () => {
    const weak = extractWeakSamples(mockReport, 'skill');
    const s003 = weak.find((s) => s.sample_id === 's003');
