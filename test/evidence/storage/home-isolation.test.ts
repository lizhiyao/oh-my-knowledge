 ...
 * 全局产物目录(reports / doctors / observe-health / observe inbox / state)必须都落在 temp,绝不指向真实 ~/.oh-my-knowledge。
...
import { globalReportsDir } from '../../../src/evidence/storage/directories.js';
import { globalObserveHealthDir, globalDoctorsDir } from '../../../src/evidence/storage/directories.js';
import { globalObservationsDir } from '../../../src/observability/inbox/paths.js';
...
describe('OMK_HOME 测试隔离守卫', () => {
  ...
  it('全局产物目录(reports / doctors / observe-health / observe inbox)都不落真实 home', () => {
    for (const d of [globalReportsDir(), globalDoctorsDir(), globalObserveHealthDir(), globalObservationsDir()]) {
