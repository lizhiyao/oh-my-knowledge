...
`.trim();
    ...
    assert.equal(result[0].assertions.length, 2);
    assert.equal(result[0].assertions[0].type, 'contains');
    assert.equal(result[0].assertions[0].value, 'SQL');
    assert.equal(result[0].assertions[0].weight, 1);
    assert.equal(result[0].assertions[1].type, 'min_length');
    assert.equal(result[0].assertions[1].value, 50);
    assert.equal(result[0].dimensions.security, 'Check for vulnerabilities');
