...
const SCHEMA_CATALOG_DOCS = [
  {
    ...
    counts: (total: number, v2: number, v1: number) =>
      `The catalog contains ${total} root contract names; ${v2} current roots use v2 and ${v1} use v1.`,
  },
  {
    path: 'docs/zh/reference/embedded-api.md',
    counts: (total: number, v2: number, v1: number) =>
      `Catalog 共包含 ${total} 个根契约名称；当前有 ${v2} 个根契约使用 v2，${v1} 个使用 v1。`,
  },
] as const;
...
describe('current evaluation contract documentation', () => {
  ...
  it('tracks the generated wire-schema catalog counts in both languages', () => {
    ...
    for (const { path, counts: expectedCounts } of SCHEMA_CATALOG_DOCS) {
      const source = readFileSync(resolve(path), 'utf8');
      expect(source).toContain(expectedCounts(WIRE_SCHEMA_CATALOG.length, v2, v1));
