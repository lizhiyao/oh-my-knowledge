/**
 * Packaging smoke test — sandbox mock 主能力依赖 src/executors/mock-runtime/mock-hook.cjs
 * 这份 hook 是 runtime executable,tsc 不处理 .cjs 文件,build script 必须把它复制到
 * dist/executors/mock-runtime/。本测试保证:
 *   1) 源码位置:src/executors/mock-runtime/mock-hook.cjs 存在(否则 build 也会挂)
 *   2) build 产物:dist/executors/mock-runtime/mock-hook.cjs 存在(否则运行时 readMockHookTemplate 挂)
...
describe('packaging — mock-hook ships with omk package', () => {
  ...
  beforeAll(() => {
    if (!existsSync('dist/executors/mock-runtime/runtime.js')) return;
  ...
  it('源码位置存在 mock-hook.cjs', () => {
    expect(existsSync('src/executors/mock-runtime/mock-hook.cjs')).toBe(true);
  });
  ...
  it('build 后 dist 里有 mock-hook.cjs(否则运行时挂)', () => {
    ...
    expect(existsSync('dist/executors/mock-runtime/mock-hook.cjs')).toBe(true);
  });
  ...
  it('npm pack 清单含 mock-hook.cjs(否则发出去的包漏 hook)', () => {
    ...
    expect(packedPaths).toContain('dist/executors/mock-runtime/mock-hook.cjs');
  });
