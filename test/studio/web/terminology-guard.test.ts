 ...
 *  3. 不得出现「技能」——`skill` 是产品身份词，中文界面保留拉丁写法（规范 §三.9）。
 *
...
function rejectedLiteral(text: string): string | null {
  ...
  if (text.includes(FORBIDDEN_SKILL_TERM)) return '被拒译名「技能」，界面用拉丁写法 Skill';
...
describe('Studio 界面用词门禁（terminology-spec §三.8／§三.9）', () => {
  ...
  it('判据正反两侧都有牙，且扫描确实覆盖界面文案', () => {
    // 正例：带限定的来源层用词与拉丁写法的 skill 必须放过——否则下一次改名会把正确文案改坏。
    for (const ok of ['来源会话：', '选择来源会话', '暂无来源会话的复盘记录。', '采集 N 份来源会话', '各 skill 健康度', 'Skills']) {
      expect(rejectedLiteral(ok), ok).toBeNull();
    }
    // 反例：未限定用词与被拒别名必须被抓到。
    for (const bad of ['会话列表', '这个会话的提炼记录', '知识对象', '搜索知识对象', '技能']) {
