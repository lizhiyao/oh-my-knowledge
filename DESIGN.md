...
- 交互态的量测进度：悬停档与按下档都已在真实页面上用派发鼠标事件量过（70 个控件按判据零未达；注入旧浅紫会报出 11 个未达，证明扫描不是空跑）。**键盘焦点（focus-visible）在真实页面上仍未量**，那部分目前只有 `design/studio/` 原型上的证据；焦点环与 Ant Design 控件的覆盖顺序也需人工确认。
