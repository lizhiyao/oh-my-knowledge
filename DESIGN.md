...
复用 `StudioShell`。一级导航包含观测、评测和知识；观测默认进入对话，不设常驻二级导航；对话列表提供低强调的“来源与采集”入口，承载本机 Agent 识别与日志采集报告，无记录时突出“如何接入对话”。具体对话阅读页不展示来源入口。工作区列表与底部设置属于同一侧栏。用 `aria-current` 表达当前位置，并保留可见状态。导航项的默认、悬停与当前状态对应 frontmatter 中的三个 `navigation-item` 角色。
