---
name: OMK Studio
description: Studio 当前实现的设计基线与已确认改进方向；品牌紫、选中底、悬停与按下三档、状态色、页级字号与两档圆角已接入运行时，间距、边界与断点仍待收敛。
colors:
  primary: "#7753ff"
...
字体、间距、页面密度与多栏结构继续通过代表页收敛，不从生成图片反推精确 token。
...
- 间距、边界、断点与多栏结构这批规范**尚未接入运行时**，本文件 frontmatter 里的这些角色仍是现状提取值。字号（第四批）、圆角（第五批）与悬停／按下三档（第五、六批）已经接入，由 `test/architecture/studio-type-scale.test.ts` 与 `studio-color-tokens.test.ts` 钉住。
- 交互态的量测进度：悬停档与按下档都已在真实页面上用派发鼠标事件量过（70 个控件按判据零未达；注入旧浅紫会报出 11 个未达，证明扫描不是空跑）。键盘焦点也已用 Tab 逐元素量过，并**量出一个尚未修的缺口**：`summary` 与侧栏按钮走本仓库的 2px 品牌紫环（对表面 4.7:1），但品牌链接与四个一级导航项走 antd 派生的 3px 浅紫 `#DECDFF`，对表面只有 1.27～1.45:1，不达焦点指示器要求的 3:1，外偏移也从 3px 缩成 1px。机制与上一批修掉的悬停色同一类——antd 运行时注入的 `a:focus-visible` 压过了本仓库那条低优先级默认值；修法倾向同样收在主题层给出实色 outline token，而不是逐个链接补覆写，具体取值待评审。输入框则完全不走 outline（量到 `outline: 3px none`），靠边框加浅紫外阴影，按非文本 3:1 判目前达标——同一页面上三种焦点画法本身是要收敛的对象。
