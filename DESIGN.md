...
- 边界色与间距阶梯（4/8/12/16/20/24/32 → `--studio-space-*`）已接入运行时；断点与多栏结构仍未接入，本文件 frontmatter 里的这些角色仍是现状提取值。字号（第四批）、圆角（第五批）与悬停／按下三档（第五、六批）已经接入，由 `test/architecture/studio-type-scale.test.ts` 与 `studio-color-tokens.test.ts` 钉住。
...
- 仍未量到的部分：候选核对的三栏整页（只有决定头与列表行两枚片段被覆盖，7 个节点）、需要真实评测产物才会出现的空态／错误态组合，以及数据丰富页面上的全部悬停控件；报告详情页要真跑一次 `omk eval` 才有实物可看，目前只有真实组件的 SSR 渲染断言。输入框的焦点仍走 Ant Design 的边框＋浅紫外阴影（`outline: 3px none`），按非文本 3:1 判达标，但它是同一页上的第三种焦点画法——要不要连它一起收成 2px 实色环，属观感决定，留给你。
