...
**尚未收敛（明确未完成，附真实页面实测）**：本批只换到了走 token 的角色，页面上仍有大量文字色不经过它。在 `http://127.0.0.1:7799` 的三个真实页面、1440×900 视口下逐文字节点量得：

- 观测页 3 组未达，全部是硬编码 `#8893a5` 的 11–12px 小字（2.92–3.10:1）——字号比原规范假设的 12px 还小一档。
- 评测页与知识页另有 4 组与 6 组未达，来源是 **Ant Design 默认色**而不是本仓库样式：`colorLink` 仍是 `#1677ff`（3.86–4.10:1）、次要文字 `#88888a`（3.32:1）、空态描述 `#8c8c8c`（3.35:1），预设 Tag 更差（警告 1.83、成功 2.21、错误 2.99:1）。

因此「把配色换成规范」还没有完成：下一步要把这些值经 `StudioTheme` 的 token（`colorLink`、`colorTextSecondary`／`Tertiary`、Tag 的填充色）与散落的硬编码小字一起收敛，而不是继续加 CSS 覆写。这几件由 [Issue #1060](https://github.com/lizhiyao/oh-my-knowledge/issues/1060) 跟踪，本文件不声称已完成。
