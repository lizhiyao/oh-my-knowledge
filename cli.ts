  ...
  try {
    ...
    if (values.each) {
      ...
      if (filePath) {
        ...
        if (!values['no-serve']) {
          const { createReportServer } = await import('./lib/report-server.js');
...
async function handleReport(argv: string[]): Promise<void> {
  ...
  if (values.export) {
    const { createFileStore } = await import('./lib/report-store.js');
    const { renderRunDetail, renderEachRunDetail } = await import('./lib/html-renderer.js');
...
async function handleGenSamples(argv: string[]): Promise<void> {
  ...
  const { generateSamples } = await import('./lib/generator.js');
...
async function handleEvolve(argv: string[]): Promise<void> {
  ...
  const { evolveSkill } = await import('./lib/evolver.js');
