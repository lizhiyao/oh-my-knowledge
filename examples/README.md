...
| 示例 | 演示能力 | 第一条命令 |
|---|---|---|
| [skill-map-showcase](./skill-map-showcase) | 目录式 skill 结构、doctor 体检、Skill Map 图谱输入 | `cd examples/skill-map-showcase && omk doctor skills/release-readiness --static-only` |
...
- 想第一眼建立「知识图谱」心智，从 `skill-map-showcase` 开始。启用 Skill Information Graph 的构建里，同一条 doctor 命令还会生成图谱 sidecar 和 Markdown Evidence Card。
